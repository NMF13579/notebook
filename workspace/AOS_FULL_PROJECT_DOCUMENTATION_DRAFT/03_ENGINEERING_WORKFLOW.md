...
artifact_role: ENGINEERING_WORKFLOW_REVIEW_VIEW
status: DRAFT
authority: NONE
canonical_owner: docs/03_Development.md
accepted_foundation: AOS/03_ENGINEERING_PIPELINE.md
...
Документ описывает product-design-level semantics подготовки, выполнения, проверки, review, recovery и delivery будущего AOS. Normative canonical owner — [docs/03_Development.md](../../docs/03_Development.md); accepted frozen foundation — [AOS/03_ENGINEERING_PIPELINE.md](../../AOS/03_ENGINEERING_PIPELINE.md).
...
Documentation edit in `workspace/` is not runtime execution. Authoring may correct its own bounded draft during refinement/harmonization; independent audit may not repair the exact subject.
...
Compute raw-byte SHA-256 for every included artifact, create a deterministic non-self-referential manifest, run construction checks and stop at human review. Technical audit is a separate read-only action.
...
Entry requires exact current authorization. Performs one smallest bounded causal change, journals/reconciles mutations, runs authorized focused checks, writes terminal Stage Report and stops. No unrelated cleanup, hidden correction cycle, validation claim or Git delivery.
...
2. Preserve logs, exact partial state and journal.
...
The authoritative accepted regression catalog in [docs/04_Lessons.md](../../docs/04_Lessons.md) remains the source for `AUTH-*`, `STATUS-*`, `SCOPE-*`, `CLI-*`, `ENV-*`, `FREEZE-*`, `INSTALL-*`, `UPDATE-*`, `RECOVERY-*`, `REVIEW-*`, `GIT-*`, `ROUTING-*`, `CONTENT-*`, `DRIFT-*`, `ADAPTER-*`, `DOGFOOD-*`, `LESSON-*`, `IDLE-*` and `PORTABLE-*` prompts.
