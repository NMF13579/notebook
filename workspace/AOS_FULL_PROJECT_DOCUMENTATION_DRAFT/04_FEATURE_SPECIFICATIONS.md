...
**Flow/states.** Verify package/target → inventory/classify ownership → exact dry-run → show conflicts → authorize apply → atomic/journaled mutation → verify → show first command/rollback. Proposed states: `DISCOVERED → PREVIEW_READY → WAITING_AUTH → APPLYING → VERIFIED | RECOVERY_REQUIRED`; uninstall is a distinct authorized branch.

**Failures/recovery.** Wrong repo, user-owned collision, interrupted update, checksum mismatch or incomplete rollback stops apply; journal/reconcile and retain user data. Changed preview requires reauthorization.
...
**Acceptance/negative.** Dry-run zero-write; apply matches preview; repeat is idempotent; user state preserved; first-start understandable. Silent overwrite, wrong-target apply, unauthorized uninstall and incomplete verification fail.
...
## FTR-010 — Scoped execution workflow and minimal runner kernels
...
**Purpose/users.** Ensure actual mutation cannot exceed prose scope, using the smallest executor semantics rather than a broad Control Plane.

**Trigger/preconditions.** Exact accepted task outcome, fresh C-007 preview and valid unconsumed C-006 authorization exist.

**Inputs → outputs.** Authorization, preview, bounded operation and recovery/check plan → actual mutation, journal/reconciliation, focused checks, C-008 Stage Report and new candidate identity.
...
**Failures/recovery.** Unexpected path, stale preview, permission boundary, partial write or tool failure immediately stops further mutation. Preserve journal and require bounded recovery/correction; no auto-retry after boundary change.
...
**Unknown/non-goals.** Exact runner primitives/sandbox/process model. Full orchestration, distributed execution and self-heal are deferred.
...
**Inputs → outputs.** Intended scope, authorization, journal/diff, last exact identity, checks/findings/decisions/permissions → recovery package/C-012 handoff with options and exactly one recommended next action.
...
## FTR-027 — Medical and Design domain modules
...
**Purpose/users.** Support specialized regulated/creative workflows through replaceable domain modules while keeping core domain-neutral and human/specialist authority explicit.

**Trigger/preconditions.** Human selects a domain use case; domain problem, data/provider/compliance boundary, specialist role and applicable policy are identified.

**Inputs → outputs.** Neutral core contracts plus domain-specific vocabulary, data classes, constraints, providers, specialist checkpoints and acceptance → isolated domain contract/module and bounded pilot review subject.

**Flow/states.** Define domain boundary → classify regulated/sensitive decisions/data → draft domain contracts → specialist review/decision → validate isolation → bounded pilot → review. Missing specialist/policy remains `BLOCKED_AFFECTED_ACTION`.

**Failures/recovery.** Agent claims clinical/legal authority, unapproved provider/data transfer, missing specialist review or module coupling. Stop affected route, preserve data boundary and remove/disable module safely.

**Dependencies/authority.** FTR-019/026 and domain policy. Human/domain specialist owns regulated decisions; core remains neutral.

**Acceptance/negative.** Specialist decision explicit; least data exposure; core works without module; domain behavior/risks/negatives testable. No medical diagnosis authority, compliance inference or bundled domains in core.

**Unknown/non-goals.** Exact domain jobs, jurisdictions, providers, data policy and professional validation. Fully deferred until separate scoped decisions/research.
