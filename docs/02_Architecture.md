...
<a id="installed-update-contract"></a>

**Версионированное обновление установленного AOS.** Принятое пользовательское
...
- Выпущенный пакет: immutable identity/version, точный manifest файлов и digest,
  ...
  digest проверяет целостность, но сам по себе не подтверждает издателя.
- Установку: exact project/install root и его identity, installer-owned код,
  user/project-owned данные, текущую и подготовленную версии, previous version,
  отдельно expected base выбора активной версии и state/data schema.
- Операцию: выбранный пакет и target, точный preview, authority, текущее состояние
  выполнения, фактические этапы подготовки/проверки/переключения, post-check и
  recovery. Изменение пакета, target или base делает старый preview неприменимым.

Код версий хранится отдельно от изменяемых настроек, задач и Evidence.
Небольшой launcher выбирает активную проверенную версию. Например:
...
.aos/
  ...
  versions/1.1.0/
  current.json
  project/                 # пользовательские настройки, задачи и Evidence
```

Это иллюстрация разделения владения, не обязательные имена каталогов и не
разрешение переносить существующие данные. Точный layout, serialization и
совместимый переход от прежней установки определяются в implementation profile.
Запись активной версии — metadata установки, не второй C-012 owner задач.

Новая версия сначала полностью подготавливается отдельно от активной, затем
проверяется. Переключение использует единый механизм блокировки запуска и update:
не допускается ни смешанное поколение файлов в одном процессе, ни новый запуск
между проверкой отсутствия активной работы и переключением. Незавершённые
операции сначала reconciled; безопасная остановка сама не отменяет их effects.
После переключения обязательны проверка целостности и объявленный safe start
check новой версии без запуска задач обследуемого проекта.

Launcher/updater должен иметь отдельную границу обновления, позволяющую заменить
в том числе базовые файлы запуска без их изменения под работающим процессом.
Существующий запрет несовместимой entry migration нельзя просто снять: до
поддержанного перехода исходная установка сохраняется, зависимый update BLOCKED.
Точный механизм отдельного updater и его совместимость — HOW выбранного профиля.

Прерывание до переключения оставляет прежнюю версию активной. Прерывание во время
переключения или потеря ответа требует сверки actual active version, подготовленного
пакета и журнала той же операции, без нового слепого apply. Повтор завершённой
операции не создаёт ещё одно обновление. Старую версию и Evidence сохраняют;
их очистка не является скрытым этапом update.

Rollback кода допустим только в объявленной recovery boundary и при совместимости
с текущими данными. Миграция данных имеет собственные from/to, затронутые данные,
полномочия, сохранение исходного состояния и проверку восстановления. При неизвестной
совместимости возврат старого кода не объявляется безопасным. Старые задачи сохраняют
свои source/version bindings; новая версия не наследует их разрешения автоматически.

Канал доставки предоставляет пакет, а не команды к исполнению. Check/получение
не активирует пакет. Конкретный transport, доверенный источник и механизм проверки
происхождения выбираются отдельно до network integration. Существующие FTR-009/011/014
и C-015/C-016 применяются по фактическим стыкам; новый scheduler или controller не нужен.
