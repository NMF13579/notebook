...
| Z08 — Установка и расширения | Подготавливает AOS, подключает возможности и позднее принимает сторонние проекты | FTR-004, FTR-026, FTR-027, FTR-033 | Есть установка и contracts подключения; FTR-027 и FTR-033 — отложенные заглушки, платформенные профили и другие расширения не завершены |
...
до IN_DISCOVERY при сохранённом DEFERRED; заглушками остаются 027, 031 и 033. Эти признаки
...
| R09 / 031 | Какие бизнес-правила доступа нужны выбранному проекту: простые роли либо условия по объектам/атрибутам; кто разрешает действие при конфликте правил | Сначала разобрать конкретные разрешённые/запрещённые действия и разрешение конфликтов. RBAC/ABAC/engine выбирает агент по требованиям. Независимо сохраняется Core boundary; заглушка закрывается лишь отдельным заданием с наблюдаемыми access cases |
...
| R11 / 033 | Исследование стороннего проекта либо фактическое принятие его на дальнейшую работу AOS. Установка файлов guardrails не доказывает второе | Сохранить DEFERRED до конкретного проекта; предложить сначала оценку. Независимо готовится список материалов и критериев сохранности. Для подключения нужны границы будущих изменений, сохранение данных/ручного кода, критерий успешного первого journey и восстановления |
...
**Подготовка без активации:** [R09](#reference-adaptation-decisions) задаёт вопросы
о бизнес-правилах, а не о библиотеке. Permissions coding agent из reference не
подменяют доступ пользователей создаваемой системы. Сохраняется PLACEHOLDER.
...
**Подготовка без активации:** safe install из
[RF-07](05_Reference.md#reference-gap-adaptation) полезен как пример conflict preview
и сохранения файлов. Установка пакета не доказывает восстановление требований или
успешное сопровождение стороннего проекта. [R11](#reference-adaptation-decisions)
отделяет эти результаты; текущая Product boundary и PLACEHOLDER сохраняются.
