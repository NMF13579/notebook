---
document_id: AOS-ATOMIC-FUNCTION-IDEA-066
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-066
function_title: "Domain Modules"
status: DRAFT
authority: NONE
...
**Предоставляет общий каркас подключения domain-specific capabilities с отдельными policies, evidence requirements и human decisions.**

Основной наблюдаемый результат функции: **`Domain Module Contract`**.

`IDEA-066` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-066` family flow теряет конкретный механизм «Domain Modules», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Domain Module Contract` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: DOMAIN_MODULE_CONTRACT
```
...
1. Пользователь или upstream stage передаёт exact input для **Domain Modules**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-066`.
4. Создаётся **`Domain Module Contract`** с Evidence и limitations.
...
**Primary output:** `Domain Module Contract`.
...
- название **Domain Modules** и exact subject;
...
- `FR-066-01` — Функция должна выполнять только следующую ответственность: Предоставляет общий каркас подключения domain-specific capabilities с отдельными policies, evidence requirements и human decisions.
- `FR-066-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-066-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-066-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-066-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-066-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-066`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-066-01` — Пользователь понимает назначение Domain Modules без чтения implementation internals.
- `AC-066-02` — Функция создаёт ровно `Domain Module Contract` либо честный terminal report.
- `AC-066-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-066-04` — Unknowns, exclusions и blockers видимы.
- `AC-066-05` — Human authority и downstream permissions не расширены.
- `AC-066-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Domain Module Contract` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_066`
