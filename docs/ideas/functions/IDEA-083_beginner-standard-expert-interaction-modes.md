---
document_id: AOS-ATOMIC-FUNCTION-IDEA-083
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-083
function_title: "Beginner / Standard / Expert Interaction Modes"
status: DRAFT
authority: NONE
...
**Переключает Beginner / Standard / Expert presentation modes без изменения safety semantics, permissions или underlying state.**

Основной наблюдаемый результат функции: **`Interaction Mode View`**.

`IDEA-083` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-083` family flow теряет конкретный механизм «Beginner / Standard / Expert Interaction Modes», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Interaction Mode View` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: INTERACTION_MODE_VIEW
```
...
1. Пользователь или upstream stage передаёт exact input для **Beginner / Standard / Expert Interaction Modes**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-083`.
4. Создаётся **`Interaction Mode View`** с Evidence и limitations.
...
**Primary output:** `Interaction Mode View`.
...
- название **Beginner / Standard / Expert Interaction Modes** и exact subject;
...
- `FR-083-01` — Функция должна выполнять только следующую ответственность: Переключает Beginner / Standard / Expert presentation modes без изменения safety semantics, permissions или underlying state.
- `FR-083-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-083-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-083-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-083-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-083-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-083`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-083-01` — Пользователь понимает назначение Beginner / Standard / Expert Interaction Modes без чтения implementation internals.
- `AC-083-02` — Функция создаёт ровно `Interaction Mode View` либо честный terminal report.
- `AC-083-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-083-04` — Unknowns, exclusions и blockers видимы.
- `AC-083-05` — Human authority и downstream permissions не расширены.
- `AC-083-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Interaction Mode View` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_083`
