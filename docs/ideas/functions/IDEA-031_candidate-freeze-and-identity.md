---
document_id: AOS-ATOMIC-FUNCTION-IDEA-031
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-031
function_title: "Candidate Freeze and Identity"
status: DRAFT
authority: NONE
...
**Фиксирует byte-exact candidate identity, source baseline и immutable review subject, чтобы validation и review относились к одному объекту.**

Основной наблюдаемый результат функции: **`Candidate Freeze Record`**.

`IDEA-031` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-031` family flow теряет конкретный механизм «Candidate Freeze and Identity», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Candidate Freeze Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: CANDIDATE_FREEZE_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Candidate Freeze and Identity**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-031`.
4. Создаётся **`Candidate Freeze Record`** с Evidence и limitations.
...
**Primary output:** `Candidate Freeze Record`.
...
- название **Candidate Freeze and Identity** и exact subject;
...
- `FR-031-01` — Функция должна выполнять только следующую ответственность: Фиксирует byte-exact candidate identity, source baseline и immutable review subject, чтобы validation и review относились к одному объекту.
- `FR-031-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-031-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-031-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-031-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-031-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-031`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-031-01` — Пользователь понимает назначение Candidate Freeze and Identity без чтения implementation internals.
- `AC-031-02` — Функция создаёт ровно `Candidate Freeze Record` либо честный terminal report.
- `AC-031-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-031-04` — Unknowns, exclusions и blockers видимы.
- `AC-031-05` — Human authority и downstream permissions не расширены.
- `AC-031-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Candidate Freeze Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_031`
