---
document_id: AOS-ATOMIC-FUNCTION-IDEA-042
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-042
function_title: "Installer Dry-run"
status: DRAFT
authority: NONE
...
**Строит deterministic install plan без изменения файлов и показывает ownership, conflicts, planned creates/modifies/deletes и prerequisites.**

Основной наблюдаемый результат функции: **`Installer Dry-Run Plan`**.

`IDEA-042` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-042` family flow теряет конкретный механизм «Installer Dry-run», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Installer Dry-Run Plan` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: INSTALLER_DRY_RUN_PLAN
```
...
1. Пользователь или upstream stage передаёт exact input для **Installer Dry-run**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-042`.
4. Создаётся **`Installer Dry-Run Plan`** с Evidence и limitations.
...
**Primary output:** `Installer Dry-Run Plan`.
...
- название **Installer Dry-run** и exact subject;
...
- `FR-042-01` — Функция должна выполнять только следующую ответственность: Строит deterministic install plan без изменения файлов и показывает ownership, conflicts, planned creates/modifies/deletes и prerequisites.
- `FR-042-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-042-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-042-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-042-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-042-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-042`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-042-01` — Пользователь понимает назначение Installer Dry-run без чтения implementation internals.
- `AC-042-02` — Функция создаёт ровно `Installer Dry-Run Plan` либо честный terminal report.
- `AC-042-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-042-04` — Unknowns, exclusions и blockers видимы.
- `AC-042-05` — Human authority и downstream permissions не расширены.
- `AC-042-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Installer Dry-Run Plan` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_042`
