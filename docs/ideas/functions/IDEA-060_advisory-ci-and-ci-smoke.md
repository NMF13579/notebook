---
document_id: AOS-ATOMIC-FUNCTION-IDEA-060
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-060
function_title: "Advisory CI and CI Smoke"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-023]
...
**Запускает advisory CI smoke и targeted quality checks, публикуя Evidence без автоматического превращения CI PASS в approval.**

Основной наблюдаемый результат функции: **`CI Smoke Report`**.

`IDEA-060` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-060` family flow теряет конкретный механизм «Advisory CI and CI Smoke», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `CI Smoke Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: CI_SMOKE_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **Advisory CI and CI Smoke**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-060`.
4. Создаётся **`CI Smoke Report`** с Evidence и limitations.
...
**Primary output:** `CI Smoke Report`.
...
- название **Advisory CI and CI Smoke** и exact subject;
...
- `FR-060-01` — Функция должна выполнять только следующую ответственность: Запускает advisory CI smoke и targeted quality checks, публикуя Evidence без автоматического превращения CI PASS в approval.
- `FR-060-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-060-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-060-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-060-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-060-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-060`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-060-01` — Пользователь понимает назначение Advisory CI and CI Smoke без чтения implementation internals.
- `AC-060-02` — Функция создаёт ровно `CI Smoke Report` либо честный terminal report.
- `AC-060-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-060-04` — Unknowns, exclusions и blockers видимы.
- `AC-060-05` — Human authority и downstream permissions не расширены.
- `AC-060-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `CI Smoke Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_060`
