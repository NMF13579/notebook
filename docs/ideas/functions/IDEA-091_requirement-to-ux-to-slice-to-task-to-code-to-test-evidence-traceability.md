---
document_id: AOS-ATOMIC-FUNCTION-IDEA-091
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-091
function_title: "Requirement → UX → Slice → Task → Code → Test/Evidence Traceability"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-031, FTR-006, FTR-012]
...
**Поддерживает сквозную связь Requirement → UX → Slice → Task → Code → Test/Evidence и показывает разрывы coverage.**

Основной наблюдаемый результат функции: **`End-to-End Traceability Graph`**.

`IDEA-091` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-091` family flow теряет конкретный механизм «Requirement → UX → Slice → Task → Code → Test/Evidence Traceability», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `End-to-End Traceability Graph` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: END_TO_END_TRACEABILITY_GRAPH
```
...
1. Пользователь или upstream stage передаёт exact input для **Requirement → UX → Slice → Task → Code → Test/Evidence Traceability**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-091`.
4. Создаётся **`End-to-End Traceability Graph`** с Evidence и limitations.
...
**Primary output:** `End-to-End Traceability Graph`.
...
- название **Requirement → UX → Slice → Task → Code → Test/Evidence Traceability** и exact subject;
...
- `FR-091-01` — Функция должна выполнять только следующую ответственность: Поддерживает сквозную связь Requirement → UX → Slice → Task → Code → Test/Evidence и показывает разрывы coverage.
- `FR-091-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-091-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-091-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-091-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-091-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-091`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-091-01` — Пользователь понимает назначение Requirement → UX → Slice → Task → Code → Test/Evidence Traceability без чтения implementation internals.
- `AC-091-02` — Функция создаёт ровно `End-to-End Traceability Graph` либо честный terminal report.
- `AC-091-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-091-04` — Unknowns, exclusions и blockers видимы.
- `AC-091-05` — Human authority и downstream permissions не расширены.
- `AC-091-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `End-to-End Traceability Graph` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_091`
