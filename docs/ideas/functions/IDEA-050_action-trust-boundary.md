---
document_id: AOS-ATOMIC-FUNCTION-IDEA-050
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-050
function_title: "Action Trust Boundary"
status: DRAFT
authority: NONE
...
**Определяет trust boundary для каждого действия, источника и инструмента: что считается data, instruction, permission и authority.**

Основной наблюдаемый результат функции: **`Action Trust Boundary Record`**.

`IDEA-050` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-050` family flow теряет конкретный механизм «Action Trust Boundary», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Action Trust Boundary Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ACTION_TRUST_BOUNDARY_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Action Trust Boundary**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-050`.
4. Создаётся **`Action Trust Boundary Record`** с Evidence и limitations.
...
**Primary output:** `Action Trust Boundary Record`.
...
- название **Action Trust Boundary** и exact subject;
...
- `FR-050-01` — Функция должна выполнять только следующую ответственность: Определяет trust boundary для каждого действия, источника и инструмента: что считается data, instruction, permission и authority.
- `FR-050-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-050-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-050-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-050-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-050-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-050`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-050-01` — Пользователь понимает назначение Action Trust Boundary без чтения implementation internals.
- `AC-050-02` — Функция создаёт ровно `Action Trust Boundary Record` либо честный terminal report.
- `AC-050-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-050-04` — Unknowns, exclusions и blockers видимы.
- `AC-050-05` — Human authority и downstream permissions не расширены.
- `AC-050-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Action Trust Boundary Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_050`
