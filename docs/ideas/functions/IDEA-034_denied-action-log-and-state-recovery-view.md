---
document_id: AOS-ATOMIC-FUNCTION-IDEA-034
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-034
function_title: "Denied Action Log and State Recovery View"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-014, FTR-019]
...
**Показывает denied actions, причины отказа, partial state и доступные recovery choices без скрытого повторения команды.**

Основной наблюдаемый результат функции: **`Denied Action and Recovery View`**.

`IDEA-034` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-034` family flow теряет конкретный механизм «Denied Action Log and State Recovery View», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Denied Action and Recovery View` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: DENIED_ACTION_AND_RECOVERY_VIEW
```
...
1. Пользователь или upstream stage передаёт exact input для **Denied Action Log and State Recovery View**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-034`.
4. Создаётся **`Denied Action and Recovery View`** с Evidence и limitations.
...
**Primary output:** `Denied Action and Recovery View`.
...
- название **Denied Action Log and State Recovery View** и exact subject;
...
- `FR-034-01` — Функция должна выполнять только следующую ответственность: Показывает denied actions, причины отказа, partial state и доступные recovery choices без скрытого повторения команды.
- `FR-034-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-034-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-034-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-034-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-034-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-034`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-034-01` — Пользователь понимает назначение Denied Action Log and State Recovery View без чтения implementation internals.
- `AC-034-02` — Функция создаёт ровно `Denied Action and Recovery View` либо честный terminal report.
- `AC-034-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-034-04` — Unknowns, exclusions и blockers видимы.
- `AC-034-05` — Human authority и downstream permissions не расширены.
- `AC-034-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Denied Action and Recovery View` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_034`
