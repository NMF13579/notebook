---
document_id: AOS-ATOMIC-FUNCTION-IDEA-067
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-067
function_title: "Medical Domain Module"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-027
related_feature_families: [FTR-027]
...
**Добавляет medical-domain workflows только при принятых clinical, privacy, regulatory и human oversight boundaries.**

Основной наблюдаемый результат функции: **`Medical Domain Module Specification`**.

`IDEA-067` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-067` family flow теряет конкретный механизм «Medical Domain Module», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Medical Domain Module Specification` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: MEDICAL_DOMAIN_MODULE_SPECIFICATION
```
...
1. Пользователь или upstream stage передаёт exact input для **Medical Domain Module**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-067`.
4. Создаётся **`Medical Domain Module Specification`** с Evidence и limitations.
...
**Primary output:** `Medical Domain Module Specification`.
...
- название **Medical Domain Module** и exact subject;
...
- `FR-067-01` — Функция должна выполнять только следующую ответственность: Добавляет medical-domain workflows только при принятых clinical, privacy, regulatory и human oversight boundaries.
- `FR-067-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-067-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-067-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-067-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-067-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-027` — Domain Modules: Design and Medical](../features/FTR-027_domain-modules-design-and-medical.md)

Primary family layer: `Regulated/creative domain extensions`. Proposed disposition: `DEFERRED_SEPARATE_DECISIONS`.
...
Каждый event содержит `function_id=IDEA-067`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-067-01` — Пользователь понимает назначение Medical Domain Module без чтения implementation internals.
- `AC-067-02` — Функция создаёт ровно `Medical Domain Module Specification` либо честный terminal report.
- `AC-067-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-067-04` — Unknowns, exclusions и blockers видимы.
- `AC-067-05` — Human authority и downstream permissions не расширены.
- `AC-067-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Medical Domain Module Specification` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_067`
