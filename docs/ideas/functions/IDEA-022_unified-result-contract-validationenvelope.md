---
document_id: AOS-ATOMIC-FUNCTION-IDEA-022
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-022
function_title: "Unified Result Contract / ValidationEnvelope"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-011
related_feature_families: [FTR-011]
...
**Нормализует результаты checks в единый ValidationEnvelope с честными status, subject, evidence, limitations и machine-readable reason codes.**

Основной наблюдаемый результат функции: **`ValidationEnvelope`**.

`IDEA-022` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-022` family flow теряет конкретный механизм «Unified Result Contract / ValidationEnvelope», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `ValidationEnvelope` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: VALIDATIONENVELOPE
```
...
1. Пользователь или upstream stage передаёт exact input для **Unified Result Contract / ValidationEnvelope**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-022`.
4. Создаётся **`ValidationEnvelope`** с Evidence и limitations.
...
**Primary output:** `ValidationEnvelope`.
...
- название **Unified Result Contract / ValidationEnvelope** и exact subject;
...
- `FR-022-01` — Функция должна выполнять только следующую ответственность: Нормализует результаты checks в единый ValidationEnvelope с честными status, subject, evidence, limitations и machine-readable reason codes.
- `FR-022-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-022-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-022-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-022-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-022-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-011` — Unified Result Contract, AOS Validate, Doctor and Self-Test](../features/FTR-011_unified-result-contract-aos-validate-doctor-and-self-test.md)

Primary family layer: `Product Runtime support / Development Factory`. Proposed disposition: `CANDIDATE`.
...
Каждый event содержит `function_id=IDEA-022`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-022-01` — Пользователь понимает назначение Unified Result Contract / ValidationEnvelope без чтения implementation internals.
- `AC-022-02` — Функция создаёт ровно `ValidationEnvelope` либо честный terminal report.
- `AC-022-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-022-04` — Unknowns, exclusions и blockers видимы.
- `AC-022-05` — Human authority и downstream permissions не расширены.
- `AC-022-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `ValidationEnvelope` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_022`
