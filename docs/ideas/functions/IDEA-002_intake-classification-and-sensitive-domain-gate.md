---
document_id: AOS-ATOMIC-FUNCTION-IDEA-002
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-002
function_title: "Intake Classification and Sensitive-Domain Gate"
status: DRAFT
authority: NONE
...
**Классифицирует тип запроса, чувствительность данных и необходимость sensitive-domain gate до передачи контента другим модулям или providers.**

Основной наблюдаемый результат функции: **`Intake Classification Record`**.

`IDEA-002` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-002` family flow теряет конкретный механизм «Intake Classification and Sensitive-Domain Gate», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Intake Classification Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: INTAKE_CLASSIFICATION_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Intake Classification and Sensitive-Domain Gate**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-002`.
4. Создаётся **`Intake Classification Record`** с Evidence и limitations.
...
**Primary output:** `Intake Classification Record`.
...
- название **Intake Classification and Sensitive-Domain Gate** и exact subject;
...
- `FR-002-01` — Функция должна выполнять только следующую ответственность: Классифицирует тип запроса, чувствительность данных и необходимость sensitive-domain gate до передачи контента другим модулям или providers.
- `FR-002-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-002-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-002-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-002-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-002-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-002`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-002-01` — Пользователь понимает назначение Intake Classification and Sensitive-Domain Gate без чтения implementation internals.
- `AC-002-02` — Функция создаёт ровно `Intake Classification Record` либо честный terminal report.
- `AC-002-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-002-04` — Unknowns, exclusions и blockers видимы.
- `AC-002-05` — Human authority и downstream permissions не расширены.
- `AC-002-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Intake Classification Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_002`
