---
document_id: AOS-ATOMIC-FUNCTION-IDEA-081
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-081
function_title: "Problem Interview with Draft Capture"
status: DRAFT
authority: NONE
...
**Проводит focused problem interview и параллельно собирает DRAFT specification, показывая пользователю, как ответы меняют документ.**

Основной наблюдаемый результат функции: **`Problem Interview Draft`**.

`IDEA-081` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-081` family flow теряет конкретный механизм «Problem Interview with Draft Capture», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Problem Interview Draft` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: PROBLEM_INTERVIEW_DRAFT
```
...
1. Пользователь или upstream stage передаёт exact input для **Problem Interview with Draft Capture**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-081`.
4. Создаётся **`Problem Interview Draft`** с Evidence и limitations.
...
**Primary output:** `Problem Interview Draft`.
...
- название **Problem Interview with Draft Capture** и exact subject;
...
- `FR-081-01` — Функция должна выполнять только следующую ответственность: Проводит focused problem interview и параллельно собирает DRAFT specification, показывая пользователю, как ответы меняют документ.
- `FR-081-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-081-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-081-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-081-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-081-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-081`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-081-01` — Пользователь понимает назначение Problem Interview with Draft Capture без чтения implementation internals.
- `AC-081-02` — Функция создаёт ровно `Problem Interview Draft` либо честный terminal report.
- `AC-081-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-081-04` — Unknowns, exclusions и blockers видимы.
- `AC-081-05` — Human authority и downstream permissions не расширены.
- `AC-081-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Problem Interview Draft` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_081`
