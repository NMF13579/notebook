---
document_id: AOS-ATOMIC-FUNCTION-IDEA-037
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-037
function_title: "Context Priority Rules"
status: DRAFT
authority: NONE
...
**Определяет precedence и freshness rules для human decisions, canonical docs, current observations, drafts, research и legacy references.**

Основной наблюдаемый результат функции: **`Context Priority Policy`**.

`IDEA-037` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-037` family flow теряет конкретный механизм «Context Priority Rules», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Context Priority Policy` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: CONTEXT_PRIORITY_POLICY
```
...
1. Пользователь или upstream stage передаёт exact input для **Context Priority Rules**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-037`.
4. Создаётся **`Context Priority Policy`** с Evidence и limitations.
...
**Primary output:** `Context Priority Policy`.
...
- название **Context Priority Rules** и exact subject;
...
- `FR-037-01` — Функция должна выполнять только следующую ответственность: Определяет precedence и freshness rules для human decisions, canonical docs, current observations, drafts, research и legacy references.
- `FR-037-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-037-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-037-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-037-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-037-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-037`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-037-01` — Пользователь понимает назначение Context Priority Rules без чтения implementation internals.
- `AC-037-02` — Функция создаёт ровно `Context Priority Policy` либо честный terminal report.
- `AC-037-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-037-04` — Unknowns, exclusions и blockers видимы.
- `AC-037-05` — Human authority и downstream permissions не расширены.
- `AC-037-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Context Priority Policy` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_037`
