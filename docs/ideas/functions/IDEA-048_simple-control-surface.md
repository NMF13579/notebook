---
document_id: AOS-ATOMIC-FUNCTION-IDEA-048
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-048
function_title: "Simple Control Surface"
status: DRAFT
authority: NONE
...
**Даёт малый набор управляющих действий для common flows, не раскрывая пользователю внутреннюю orchestration complexity и не обходя permissions.**

Основной наблюдаемый результат функции: **`Simple Control Surface`**.

`IDEA-048` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-048` family flow теряет конкретный механизм «Simple Control Surface», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Simple Control Surface` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: SIMPLE_CONTROL_SURFACE
```
...
1. Пользователь или upstream stage передаёт exact input для **Simple Control Surface**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-048`.
4. Создаётся **`Simple Control Surface`** с Evidence и limitations.
...
**Primary output:** `Simple Control Surface`.
...
- название **Simple Control Surface** и exact subject;
...
- `FR-048-01` — Функция должна выполнять только следующую ответственность: Даёт малый набор управляющих действий для common flows, не раскрывая пользователю внутреннюю orchestration complexity и не обходя permissions.
- `FR-048-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-048-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-048-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-048-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-048-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-048`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-048-01` — Пользователь понимает назначение Simple Control Surface без чтения implementation internals.
- `AC-048-02` — Функция создаёт ровно `Simple Control Surface` либо честный terminal report.
- `AC-048-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-048-04` — Unknowns, exclusions и blockers видимы.
- `AC-048-05` — Human authority и downstream permissions не расширены.
- `AC-048-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Simple Control Surface` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_048`
