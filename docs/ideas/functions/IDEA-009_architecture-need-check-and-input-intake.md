---
document_id: AOS-ATOMIC-FUNCTION-IDEA-009
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-009
function_title: "Architecture Need Check and Input Intake"
status: DRAFT
authority: NONE
...
**Определяет, существует ли material architecture choice, и собирает входы, constraints, unknowns и affected requirements для human architecture review.**

Основной наблюдаемый результат функции: **`Architecture Need Check`**.

`IDEA-009` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-009` family flow теряет конкретный механизм «Architecture Need Check and Input Intake», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Architecture Need Check` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ARCHITECTURE_NEED_CHECK
```
...
1. Пользователь или upstream stage передаёт exact input для **Architecture Need Check and Input Intake**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-009`.
4. Создаётся **`Architecture Need Check`** с Evidence и limitations.
...
**Primary output:** `Architecture Need Check`.
...
- название **Architecture Need Check and Input Intake** и exact subject;
...
- `FR-009-01` — Функция должна выполнять только следующую ответственность: Определяет, существует ли material architecture choice, и собирает входы, constraints, unknowns и affected requirements для human architecture review.
- `FR-009-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-009-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-009-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-009-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-009-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-009`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-009-01` — Пользователь понимает назначение Architecture Need Check and Input Intake без чтения implementation internals.
- `AC-009-02` — Функция создаёт ровно `Architecture Need Check` либо честный terminal report.
- `AC-009-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-009-04` — Unknowns, exclusions и blockers видимы.
- `AC-009-05` — Human authority и downstream permissions не расширены.
- `AC-009-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Architecture Need Check` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_009`
