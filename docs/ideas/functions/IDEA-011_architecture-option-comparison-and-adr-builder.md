---
document_id: AOS-ATOMIC-FUNCTION-IDEA-011
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-011
function_title: "Architecture Option Comparison and ADR Builder"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-005]
...
**Сравнивает architecture options по trade-offs, risks, reversibility, compatibility и testability и подготавливает ADR candidate.**

Основной наблюдаемый результат функции: **`Architecture Option Matrix and ADR Draft`**.

`IDEA-011` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-011` family flow теряет конкретный механизм «Architecture Option Comparison and ADR Builder», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Architecture Option Matrix and ADR Draft` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ARCHITECTURE_OPTION_MATRIX_AND_ADR_DRAFT
```
...
1. Пользователь или upstream stage передаёт exact input для **Architecture Option Comparison and ADR Builder**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-011`.
4. Создаётся **`Architecture Option Matrix and ADR Draft`** с Evidence и limitations.
...
**Primary output:** `Architecture Option Matrix and ADR Draft`.
...
- название **Architecture Option Comparison and ADR Builder** и exact subject;
...
- `FR-011-01` — Функция должна выполнять только следующую ответственность: Сравнивает architecture options по trade-offs, risks, reversibility, compatibility и testability и подготавливает ADR candidate.
- `FR-011-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-011-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-011-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-011-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-011-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-011`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-011-01` — Пользователь понимает назначение Architecture Option Comparison and ADR Builder без чтения implementation internals.
- `AC-011-02` — Функция создаёт ровно `Architecture Option Matrix and ADR Draft` либо честный terminal report.
- `AC-011-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-011-04` — Unknowns, exclusions и blockers видимы.
- `AC-011-05` — Human authority и downstream permissions не расширены.
- `AC-011-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Architecture Option Matrix and ADR Draft` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_011`
