---
document_id: AOS-ATOMIC-FUNCTION-IDEA-059
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-059
function_title: "Registry / Drift, Source-of-Truth Guard and Human Decision Authenticity"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-021
related_feature_families: [FTR-021]
...
**Обнаруживает registry/schema/runtime drift, competing Source-of-Truth claims и поддельные/непривязанные human decisions.**

Основной наблюдаемый результат функции: **`Integrity and Authenticity Audit`**.

`IDEA-059` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-059` family flow теряет конкретный механизм «Registry / Drift, Source-of-Truth Guard and Human Decision Authenticity», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Integrity and Authenticity Audit` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: INTEGRITY_AND_AUTHENTICITY_AUDIT
```
...
1. Пользователь или upstream stage передаёт exact input для **Registry / Drift, Source-of-Truth Guard and Human Decision Authenticity**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-059`.
4. Создаётся **`Integrity and Authenticity Audit`** с Evidence и limitations.
...
**Primary output:** `Integrity and Authenticity Audit`.
...
- название **Registry / Drift, Source-of-Truth Guard and Human Decision Authenticity** и exact subject;
...
- `FR-059-01` — Функция должна выполнять только следующую ответственность: Обнаруживает registry/schema/runtime drift, competing Source-of-Truth claims и поддельные/непривязанные human decisions.
- `FR-059-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-059-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-059-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-059-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-059-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-021` — Registry/Drift Detection, Source-of-Truth Guard and Human-Decision Authenticity](../features/FTR-021_registry-drift-detection-source-of-truth-guard-and-human-decision-authenticity.md)

Primary family layer: `Development Factory / Governance support`. Proposed disposition: `DEFERRED_AFTER_MANUAL_FLOW`.
...
Каждый event содержит `function_id=IDEA-059`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-059-01` — Пользователь понимает назначение Registry / Drift, Source-of-Truth Guard and Human Decision Authenticity без чтения implementation internals.
- `AC-059-02` — Функция создаёт ровно `Integrity and Authenticity Audit` либо честный terminal report.
- `AC-059-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-059-04` — Unknowns, exclusions и blockers видимы.
- `AC-059-05` — Human authority и downstream permissions не расширены.
- `AC-059-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Integrity and Authenticity Audit` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_059`
