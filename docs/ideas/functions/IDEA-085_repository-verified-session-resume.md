---
document_id: AOS-ATOMIC-FUNCTION-IDEA-085
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-085
function_title: "Repository-Verified Session Resume: «На чём мы остановились?»"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-008, FTR-014, FTR-016]
...
**Отвечает на вопрос «На чём мы остановились?» только после проверки repository, branch, HEAD, status/diff и durable project records.**

Основной наблюдаемый результат функции: **`Repository-Verified Resume Summary`**.

`IDEA-085` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-085` family flow теряет конкретный механизм «Repository-Verified Session Resume: «На чём мы остановились?»», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Repository-Verified Resume Summary` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: REPOSITORY_VERIFIED_RESUME_SUMMARY
```
...
1. Пользователь или upstream stage передаёт exact input для **Repository-Verified Session Resume: «На чём мы остановились?»**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-085`.
4. Создаётся **`Repository-Verified Resume Summary`** с Evidence и limitations.
...
**Primary output:** `Repository-Verified Resume Summary`.
...
- название **Repository-Verified Session Resume: «На чём мы остановились?»** и exact subject;
...
- `FR-085-01` — Функция должна выполнять только следующую ответственность: Отвечает на вопрос «На чём мы остановились?» только после проверки repository, branch, HEAD, status/diff и durable project records.
- `FR-085-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-085-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-085-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-085-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-085-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-085`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-085-01` — Пользователь понимает назначение Repository-Verified Session Resume: «На чём мы остановились?» без чтения implementation internals.
- `AC-085-02` — Функция создаёт ровно `Repository-Verified Resume Summary` либо честный terminal report.
- `AC-085-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-085-04` — Unknowns, exclusions и blockers видимы.
- `AC-085-05` — Human authority и downstream permissions не расширены.
- `AC-085-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Repository-Verified Resume Summary` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_085`
