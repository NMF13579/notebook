---
document_id: AOS-ATOMIC-FUNCTION-IDEA-079
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-079
function_title: "Auto-Scaling Intake Mode"
status: DRAFT
authority: NONE
...
**Автоматически выбирает краткий или расширенный intake flow по достаточности входа, не заставляя готовое ТЗ проходить полный interview.**

Основной наблюдаемый результат функции: **`Auto-Scaling Intake Route`**.

`IDEA-079` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-079` family flow теряет конкретный механизм «Auto-Scaling Intake Mode», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Auto-Scaling Intake Route` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: AUTO_SCALING_INTAKE_ROUTE
```
...
1. Пользователь или upstream stage передаёт exact input для **Auto-Scaling Intake Mode**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-079`.
4. Создаётся **`Auto-Scaling Intake Route`** с Evidence и limitations.
...
**Primary output:** `Auto-Scaling Intake Route`.
...
- название **Auto-Scaling Intake Mode** и exact subject;
...
- `FR-079-01` — Функция должна выполнять только следующую ответственность: Автоматически выбирает краткий или расширенный intake flow по достаточности входа, не заставляя готовое ТЗ проходить полный interview.
- `FR-079-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-079-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-079-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-079-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-079-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-079`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-079-01` — Пользователь понимает назначение Auto-Scaling Intake Mode без чтения implementation internals.
- `AC-079-02` — Функция создаёт ровно `Auto-Scaling Intake Route` либо честный terminal report.
- `AC-079-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-079-04` — Unknowns, exclusions и blockers видимы.
- `AC-079-05` — Human authority и downstream permissions не расширены.
- `AC-079-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Auto-Scaling Intake Route` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_079`
