---
document_id: AOS-ATOMIC-FUNCTION-IDEA-004
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-004
function_title: "Project Discovery and Capability Map"
status: DRAFT
authority: NONE
...
**Выполняет bounded read-only обследование проекта и формирует карту реально обнаруженных capabilities, entrypoints, contracts, commands, dependencies и tests.**

Основной наблюдаемый результат функции: **`Capability Map`**.

`IDEA-004` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-004` family flow теряет конкретный механизм «Project Discovery and Capability Map», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Capability Map` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: CAPABILITY_MAP
```
...
1. Пользователь или upstream stage передаёт exact input для **Project Discovery and Capability Map**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-004`.
4. Создаётся **`Capability Map`** с Evidence и limitations.
...
- название **Project Discovery and Capability Map** и exact subject;
...
- `FR-004-01` — Функция должна выполнять только следующую ответственность: Выполняет bounded read-only обследование проекта и формирует карту реально обнаруженных capabilities, entrypoints, contracts, commands, dependencies и tests.
- `FR-004-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-004-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-004-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-004-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-004-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-004`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-004-01` — Пользователь понимает назначение Project Discovery and Capability Map без чтения implementation internals.
- `AC-004-02` — Функция создаёт ровно `Capability Map` либо честный terminal report.
- `AC-004-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-004-04` — Unknowns, exclusions и blockers видимы.
- `AC-004-05` — Human authority и downstream permissions не расширены.
- `AC-004-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Capability Map` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_004`
