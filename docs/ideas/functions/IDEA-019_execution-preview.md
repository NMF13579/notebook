---
document_id: AOS-ATOMIC-FUNCTION-IDEA-019
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-019
function_title: "Execution Preview"
status: DRAFT
authority: NONE
...
**Показывает exact plan будущего действия: files, commands, creates/modifies/deletes, conflicts, permissions и stop conditions до authorization.**

Основной наблюдаемый результат функции: **`Execution Preview`**.

`IDEA-019` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-019` family flow теряет конкретный механизм «Execution Preview», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Execution Preview` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: EXECUTION_PREVIEW
```
...
1. Пользователь или upstream stage передаёт exact input для **Execution Preview**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-019`.
4. Создаётся **`Execution Preview`** с Evidence и limitations.
...
**Primary output:** `Execution Preview`.
...
- название **Execution Preview** и exact subject;
...
- `FR-019-01` — Функция должна выполнять только следующую ответственность: Показывает exact plan будущего действия: files, commands, creates/modifies/deletes, conflicts, permissions и stop conditions до authorization.
- `FR-019-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-019-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-019-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-019-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-019-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-019`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-019-01` — Пользователь понимает назначение Execution Preview без чтения implementation internals.
- `AC-019-02` — Функция создаёт ровно `Execution Preview` либо честный terminal report.
- `AC-019-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-019-04` — Unknowns, exclusions и blockers видимы.
- `AC-019-05` — Human authority и downstream permissions не расширены.
- `AC-019-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Execution Preview` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_019`
