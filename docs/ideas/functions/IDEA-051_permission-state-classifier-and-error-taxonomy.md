---
document_id: AOS-ATOMIC-FUNCTION-IDEA-051
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-051
function_title: "Permission State Classifier and Error Taxonomy"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-019]
...
**Классифицирует permission states и errors в понятную taxonomy, отличая denied, unavailable, unsafe, invalid, unknown и retryable conditions.**

Основной наблюдаемый результат функции: **`Permission State and Error Classification`**.

`IDEA-051` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-051` family flow теряет конкретный механизм «Permission State Classifier and Error Taxonomy», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Permission State and Error Classification` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: PERMISSION_STATE_AND_ERROR_CLASSIFICATION
```
...
1. Пользователь или upstream stage передаёт exact input для **Permission State Classifier and Error Taxonomy**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-051`.
4. Создаётся **`Permission State and Error Classification`** с Evidence и limitations.
...
**Primary output:** `Permission State and Error Classification`.
...
- название **Permission State Classifier and Error Taxonomy** и exact subject;
...
- `FR-051-01` — Функция должна выполнять только следующую ответственность: Классифицирует permission states и errors в понятную taxonomy, отличая denied, unavailable, unsafe, invalid, unknown и retryable conditions.
- `FR-051-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-051-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-051-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-051-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-051-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-051`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-051-01` — Пользователь понимает назначение Permission State Classifier and Error Taxonomy без чтения implementation internals.
- `AC-051-02` — Функция создаёт ровно `Permission State and Error Classification` либо честный terminal report.
- `AC-051-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-051-04` — Unknowns, exclusions и blockers видимы.
- `AC-051-05` — Human authority и downstream permissions не расширены.
- `AC-051-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Permission State and Error Classification` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_051`
