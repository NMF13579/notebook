---
document_id: AOS-ATOMIC-FUNCTION-IDEA-096
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-096
function_title: "Evidence Decay and Revalidation Trigger Manager"
status: DRAFT
authority: NONE
...
# IDEA-096 — Evidence Decay and Revalidation Trigger Manager
...
**Отслеживает freshness evidence, decay conditions и revalidation triggers, запрещая silent reuse устаревшего основания.**

Основной наблюдаемый результат функции: **`Evidence Revalidation Notice`**.

`IDEA-096` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-096` family flow теряет конкретный механизм «Evidence Decay and Revalidation Trigger Manager», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Evidence Revalidation Notice` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: EVIDENCE_REVALIDATION_NOTICE
```
...
1. Пользователь или upstream stage передаёт exact input для **Evidence Decay and Revalidation Trigger Manager**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-096`.
4. Создаётся **`Evidence Revalidation Notice`** с Evidence и limitations.
...
**Primary output:** `Evidence Revalidation Notice`.
...
- название **Evidence Decay and Revalidation Trigger Manager** и exact subject;
...
- `FR-096-01` — Функция должна выполнять только следующую ответственность: Отслеживает freshness evidence, decay conditions и revalidation triggers, запрещая silent reuse устаревшего основания.
- `FR-096-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-096-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-096-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-096-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-096-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-096`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-096-01` — Пользователь понимает назначение Evidence Decay and Revalidation Trigger Manager без чтения implementation internals.
- `AC-096-02` — Функция создаёт ровно `Evidence Revalidation Notice` либо честный terminal report.
- `AC-096-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-096-04` — Unknowns, exclusions и blockers видимы.
- `AC-096-05` — Human authority и downstream permissions не расширены.
- `AC-096-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `Evidence Revalidation Notice` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_096`
