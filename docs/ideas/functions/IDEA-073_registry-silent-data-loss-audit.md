---
document_id: AOS-ATOMIC-FUNCTION-IDEA-073
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-073
function_title: "Registry Silent Data-Loss Audit"
status: DRAFT
authority: NONE
...
**Проверяет registries на silent data loss, ignored fields, ambiguous merges и mismatch между declared и loaded state.**

Основной наблюдаемый результат функции: **`Registry Data-Loss Audit`**.

`IDEA-073` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-073` family flow теряет конкретный механизм «Registry Silent Data-Loss Audit», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Registry Data-Loss Audit` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: REGISTRY_DATA_LOSS_AUDIT
```
...
1. Пользователь или upstream stage передаёт exact input для **Registry Silent Data-Loss Audit**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-073`.
4. Создаётся **`Registry Data-Loss Audit`** с Evidence и limitations.
...
**Primary output:** `Registry Data-Loss Audit`.
...
- название **Registry Silent Data-Loss Audit** и exact subject;
...
- `FR-073-01` — Функция должна выполнять только следующую ответственность: Проверяет registries на silent data loss, ignored fields, ambiguous merges и mismatch между declared и loaded state.
- `FR-073-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-073-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-073-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-073-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-073-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-073`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-073-01` — Пользователь понимает назначение Registry Silent Data-Loss Audit без чтения implementation internals.
- `AC-073-02` — Функция создаёт ровно `Registry Data-Loss Audit` либо честный terminal report.
- `AC-073-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-073-04` — Unknowns, exclusions и blockers видимы.
- `AC-073-05` — Human authority и downstream permissions не расширены.
- `AC-073-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Registry Data-Loss Audit` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_073`
