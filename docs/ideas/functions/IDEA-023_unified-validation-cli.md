---
document_id: AOS-ATOMIC-FUNCTION-IDEA-023
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-023
function_title: "Unified Validation CLI"
status: DRAFT
authority: NONE
...
**Предоставляет единый CLI entrypoint для targeted validation, сохраняя стабильный output contract и ненулевые exit codes при failure.**

Основной наблюдаемый результат функции: **`Validation CLI Result`**.

`IDEA-023` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-023` family flow теряет конкретный механизм «Unified Validation CLI», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Validation CLI Result` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: VALIDATION_CLI_RESULT
```
...
1. Пользователь или upstream stage передаёт exact input для **Unified Validation CLI**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-023`.
4. Создаётся **`Validation CLI Result`** с Evidence и limitations.
...
**Primary output:** `Validation CLI Result`.
...
- название **Unified Validation CLI** и exact subject;
...
- `FR-023-01` — Функция должна выполнять только следующую ответственность: Предоставляет единый CLI entrypoint для targeted validation, сохраняя стабильный output contract и ненулевые exit codes при failure.
- `FR-023-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-023-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-023-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-023-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-023-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-023`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-023-01` — Пользователь понимает назначение Unified Validation CLI без чтения implementation internals.
- `AC-023-02` — Функция создаёт ровно `Validation CLI Result` либо честный terminal report.
- `AC-023-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-023-04` — Unknowns, exclusions и blockers видимы.
- `AC-023-05` — Human authority и downstream permissions не расширены.
- `AC-023-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Validation CLI Result` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_023`
