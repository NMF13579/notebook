---
document_id: AOS-ATOMIC-FUNCTION-IDEA-100
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-100
function_title: "Visual-Direction Snapshot"
status: DRAFT
authority: NONE
...
# IDEA-100 — Visual-Direction Snapshot
...
**Фиксирует reviewed visual direction, approved/excluded scope и exact contract revision, не выдавая snapshot за implementation acceptance.**

Основной наблюдаемый результат функции: **`Visual-Direction Snapshot`**.

`IDEA-100` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-100` family flow теряет конкретный механизм «Visual-Direction Snapshot», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Visual-Direction Snapshot` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: VISUAL_DIRECTION_SNAPSHOT
```
...
1. Пользователь или upstream stage передаёт exact input для **Visual-Direction Snapshot**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-100`.
4. Создаётся **`Visual-Direction Snapshot`** с Evidence и limitations.
...
**Primary output:** `Visual-Direction Snapshot`.
...
- название **Visual-Direction Snapshot** и exact subject;
...
- `FR-100-01` — Функция должна выполнять только следующую ответственность: Фиксирует reviewed visual direction, approved/excluded scope и exact contract revision, не выдавая snapshot за implementation acceptance.
- `FR-100-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-100-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-100-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-100-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-100-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-100`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-100-01` — Пользователь понимает назначение Visual-Direction Snapshot без чтения implementation internals.
- `AC-100-02` — Функция создаёт ровно `Visual-Direction Snapshot` либо честный terminal report.
- `AC-100-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-100-04` — Unknowns, exclusions и blockers видимы.
- `AC-100-05` — Human authority и downstream permissions не расширены.
- `AC-100-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `Visual-Direction Snapshot` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_100`
