---
document_id: AOS-ATOMIC-FUNCTION-IDEA-046
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-046
function_title: "Example Project and Onboarding"
status: DRAFT
authority: NONE
...
**Предоставляет минимальный example project и onboarding path, показывающий первый полезный flow без скрытой complexity.**

Основной наблюдаемый результат функции: **`Example Project and Onboarding Guide`**.

`IDEA-046` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-046` family flow теряет конкретный механизм «Example Project and Onboarding», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Example Project and Onboarding Guide` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: EXAMPLE_PROJECT_AND_ONBOARDING_GUIDE
```
...
1. Пользователь или upstream stage передаёт exact input для **Example Project and Onboarding**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-046`.
4. Создаётся **`Example Project and Onboarding Guide`** с Evidence и limitations.
...
**Primary output:** `Example Project and Onboarding Guide`.
...
- название **Example Project and Onboarding** и exact subject;
...
- `FR-046-01` — Функция должна выполнять только следующую ответственность: Предоставляет минимальный example project и onboarding path, показывающий первый полезный flow без скрытой complexity.
- `FR-046-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-046-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-046-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-046-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-046-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-046`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-046-01` — Пользователь понимает назначение Example Project and Onboarding без чтения implementation internals.
- `AC-046-02` — Функция создаёт ровно `Example Project and Onboarding Guide` либо честный terminal report.
- `AC-046-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-046-04` — Unknowns, exclusions и blockers видимы.
- `AC-046-05` — Human authority и downstream permissions не расширены.
- `AC-046-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Example Project and Onboarding Guide` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_046`
