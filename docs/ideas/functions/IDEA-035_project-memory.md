---
document_id: AOS-ATOMIC-FUNCTION-IDEA-035
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-035
function_title: "Project Memory"
status: DRAFT
authority: NONE
...
**Хранит durable project knowledge, decisions, current stage и unresolved items с provenance и freshness, не заменяя canonical owners фактов.**

Основной наблюдаемый результат функции: **`Project Memory Record`**.

`IDEA-035` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-035` family flow теряет конкретный механизм «Project Memory», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Project Memory Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: PROJECT_MEMORY_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Project Memory**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-035`.
4. Создаётся **`Project Memory Record`** с Evidence и limitations.
...
**Primary output:** `Project Memory Record`.
...
- название **Project Memory** и exact subject;
...
- `FR-035-01` — Функция должна выполнять только следующую ответственность: Хранит durable project knowledge, decisions, current stage и unresolved items с provenance и freshness, не заменяя canonical owners фактов.
- `FR-035-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-035-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-035-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-035-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-035-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-035`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-035-01` — Пользователь понимает назначение Project Memory без чтения implementation internals.
- `AC-035-02` — Функция создаёт ровно `Project Memory Record` либо честный terminal report.
- `AC-035-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-035-04` — Unknowns, exclusions и blockers видимы.
- `AC-035-05` — Human authority и downstream permissions не расширены.
- `AC-035-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Project Memory Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_035`
