---
document_id: AOS-ATOMIC-FUNCTION-IDEA-007
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-007
function_title: "Technical Assignment"
status: DRAFT
authority: NONE
...
**Формирует техническую постановку там, где product specification недостаточно для последующей architecture или task decomposition, не превращая её в execution authorization.**

Основной наблюдаемый результат функции: **`Technical Assignment`**.

`IDEA-007` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-007` family flow теряет конкретный механизм «Technical Assignment», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Technical Assignment` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: TECHNICAL_ASSIGNMENT
```
...
1. Пользователь или upstream stage передаёт exact input для **Technical Assignment**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-007`.
4. Создаётся **`Technical Assignment`** с Evidence и limitations.
...
**Primary output:** `Technical Assignment`.
...
- название **Technical Assignment** и exact subject;
...
- `FR-007-01` — Функция должна выполнять только следующую ответственность: Формирует техническую постановку там, где product specification недостаточно для последующей architecture или task decomposition, не превращая её в execution authorization.
- `FR-007-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-007-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-007-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-007-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-007-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-007`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-007-01` — Пользователь понимает назначение Technical Assignment без чтения implementation internals.
- `AC-007-02` — Функция создаёт ровно `Technical Assignment` либо честный terminal report.
- `AC-007-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-007-04` — Unknowns, exclusions и blockers видимы.
- `AC-007-05` — Human authority и downstream permissions не расширены.
- `AC-007-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Technical Assignment` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_007`
