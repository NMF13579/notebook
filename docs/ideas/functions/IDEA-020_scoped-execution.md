---
document_id: AOS-ATOMIC-FUNCTION-IDEA-020
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-020
function_title: "Scoped Execution"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-010]
...
**Выполняет только один явно разрешённый bounded mutation в заданном subject и прекращает работу после result/failure.**

Основной наблюдаемый результат функции: **`Scoped Execution Result`**.

`IDEA-020` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-020` family flow теряет конкретный механизм «Scoped Execution», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Scoped Execution Result` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: SCOPED_EXECUTION_RESULT
```
...
1. Пользователь или upstream stage передаёт exact input для **Scoped Execution**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-020`.
4. Создаётся **`Scoped Execution Result`** с Evidence и limitations.
...
**Primary output:** `Scoped Execution Result`.
...
- название **Scoped Execution** и exact subject;
...
- `FR-020-01` — Функция должна выполнять только следующую ответственность: Выполняет только один явно разрешённый bounded mutation в заданном subject и прекращает работу после result/failure.
- `FR-020-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-020-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-020-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-020-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-020-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-020`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-020-01` — Пользователь понимает назначение Scoped Execution без чтения implementation internals.
- `AC-020-02` — Функция создаёт ровно `Scoped Execution Result` либо честный terminal report.
- `AC-020-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-020-04` — Unknowns, exclusions и blockers видимы.
- `AC-020-05` — Human authority и downstream permissions не расширены.
- `AC-020-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Scoped Execution Result` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_020`
