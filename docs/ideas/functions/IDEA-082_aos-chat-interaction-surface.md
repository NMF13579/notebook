---
document_id: AOS-ATOMIC-FUNCTION-IDEA-082
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-082
function_title: "AOS Chat Interaction Surface"
status: DRAFT
authority: NONE
...
**Организует chat-first взаимодействие человека с AOS: context, cards, actions, confirmations, progress и closure поверх canonical records.**

Основной наблюдаемый результат функции: **`AOS Chat Interaction Session`**.

`IDEA-082` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-082` family flow теряет конкретный механизм «AOS Chat Interaction Surface», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `AOS Chat Interaction Session` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: AOS_CHAT_INTERACTION_SESSION
```
...
1. Пользователь или upstream stage передаёт exact input для **AOS Chat Interaction Surface**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-082`.
4. Создаётся **`AOS Chat Interaction Session`** с Evidence и limitations.
...
**Primary output:** `AOS Chat Interaction Session`.
...
- название **AOS Chat Interaction Surface** и exact subject;
...
- `FR-082-01` — Функция должна выполнять только следующую ответственность: Организует chat-first взаимодействие человека с AOS: context, cards, actions, confirmations, progress и closure поверх canonical records.
- `FR-082-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-082-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-082-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-082-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-082-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-082`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-082-01` — Пользователь понимает назначение AOS Chat Interaction Surface без чтения implementation internals.
- `AC-082-02` — Функция создаёт ровно `AOS Chat Interaction Session` либо честный terminal report.
- `AC-082-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-082-04` — Unknowns, exclusions и blockers видимы.
- `AC-082-05` — Human authority и downstream permissions не расширены.
- `AC-082-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `AOS Chat Interaction Session` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_082`
