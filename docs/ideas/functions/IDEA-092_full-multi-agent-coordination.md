---
document_id: AOS-ATOMIC-FUNCTION-IDEA-092
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-092
function_title: "Full Multi-Agent Coordination"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-018, FTR-026]
...
**Координирует несколько agents по явным ролям, shared contracts, dependency graph, conflict handling и human checkpoints без автономного расширения scope.**

Основной наблюдаемый результат функции: **`Multi-Agent Coordination Plan`**.

`IDEA-092` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-092` family flow теряет конкретный механизм «Full Multi-Agent Coordination», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Multi-Agent Coordination Plan` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: MULTI_AGENT_COORDINATION_PLAN
```
...
1. Пользователь или upstream stage передаёт exact input для **Full Multi-Agent Coordination**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-092`.
4. Создаётся **`Multi-Agent Coordination Plan`** с Evidence и limitations.
...
**Primary output:** `Multi-Agent Coordination Plan`.
...
- название **Full Multi-Agent Coordination** и exact subject;
...
- `FR-092-01` — Функция должна выполнять только следующую ответственность: Координирует несколько agents по явным ролям, shared contracts, dependency graph, conflict handling и human checkpoints без автономного расширения scope.
- `FR-092-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-092-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-092-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-092-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-092-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-092`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-092-01` — Пользователь понимает назначение Full Multi-Agent Coordination без чтения implementation internals.
- `AC-092-02` — Функция создаёт ровно `Multi-Agent Coordination Plan` либо честный terminal report.
- `AC-092-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-092-04` — Unknowns, exclusions и blockers видимы.
- `AC-092-05` — Human authority и downstream permissions не расширены.
- `AC-092-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Multi-Agent Coordination Plan` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_092`
