---
document_id: AOS-ATOMIC-FUNCTION-IDEA-094
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-094
function_title: "ADI Reasoning Cycle: Abduction → Deduction → Induction"
status: DRAFT
authority: NONE
...
# IDEA-094 — ADI Reasoning Cycle: Abduction → Deduction → Induction
...
**Применяет компактный ADI loop: формирует hypotheses, выводит проверяемые consequences и обновляет confidence по observations.**

Основной наблюдаемый результат функции: **`ADI Reasoning Record`**.

`IDEA-094` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-094` family flow теряет конкретный механизм «ADI Reasoning Cycle: Abduction → Deduction → Induction», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `ADI Reasoning Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ADI_REASONING_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **ADI Reasoning Cycle: Abduction → Deduction → Induction**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-094`.
4. Создаётся **`ADI Reasoning Record`** с Evidence и limitations.
...
**Primary output:** `ADI Reasoning Record`.
...
- название **ADI Reasoning Cycle: Abduction → Deduction → Induction** и exact subject;
...
- `FR-094-01` — Функция должна выполнять только следующую ответственность: Применяет компактный ADI loop: формирует hypotheses, выводит проверяемые consequences и обновляет confidence по observations.
- `FR-094-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-094-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-094-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-094-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-094-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-094`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-094-01` — Пользователь понимает назначение ADI Reasoning Cycle: Abduction → Deduction → Induction без чтения implementation internals.
- `AC-094-02` — Функция создаёт ровно `ADI Reasoning Record` либо честный terminal report.
- `AC-094-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-094-04` — Unknowns, exclusions и blockers видимы.
- `AC-094-05` — Human authority и downstream permissions не расширены.
- `AC-094-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `ADI Reasoning Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_094`
