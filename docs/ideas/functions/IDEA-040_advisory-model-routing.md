---
document_id: AOS-ATOMIC-FUNCTION-IDEA-040
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-040
function_title: "Advisory Model Routing"
status: DRAFT
authority: NONE
...
**Рекомендует подходящий model/provider для задачи на основании capability, sensitivity, cost и measured quality, не выдавая route за authorization.**

Основной наблюдаемый результат функции: **`Model Routing Recommendation`**.

`IDEA-040` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-040` family flow теряет конкретный механизм «Advisory Model Routing», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Model Routing Recommendation` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: MODEL_ROUTING_RECOMMENDATION
```
...
1. Пользователь или upstream stage передаёт exact input для **Advisory Model Routing**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-040`.
4. Создаётся **`Model Routing Recommendation`** с Evidence и limitations.
...
**Primary output:** `Model Routing Recommendation`.
...
- название **Advisory Model Routing** и exact subject;
...
- `FR-040-01` — Функция должна выполнять только следующую ответственность: Рекомендует подходящий model/provider для задачи на основании capability, sensitivity, cost и measured quality, не выдавая route за authorization.
- `FR-040-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-040-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-040-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-040-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-040-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-040`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-040-01` — Пользователь понимает назначение Advisory Model Routing без чтения implementation internals.
- `AC-040-02` — Функция создаёт ровно `Model Routing Recommendation` либо честный terminal report.
- `AC-040-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-040-04` — Unknowns, exclusions и blockers видимы.
- `AC-040-05` — Human authority и downstream permissions не расширены.
- `AC-040-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Model Routing Recommendation` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_040`
