---
document_id: AOS-ATOMIC-FUNCTION-IDEA-087
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-087
function_title: "Idea Deduplication and Promotion to DRAFT Task"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-006, FTR-007]
...
**Находит duplicate/overlap, dependencies и conflicts между ideas и формирует promotion candidate в Feature Contract или DRAFT Task только для human choice.**

Основной наблюдаемый результат функции: **`Idea Deduplication and Promotion Proposal`**.

`IDEA-087` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-087` family flow теряет конкретный механизм «Idea Deduplication and Promotion to DRAFT Task», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Idea Deduplication and Promotion Proposal` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: IDEA_DEDUPLICATION_AND_PROMOTION_PROPOSAL
```
...
1. Пользователь или upstream stage передаёт exact input для **Idea Deduplication and Promotion to DRAFT Task**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-087`.
4. Создаётся **`Idea Deduplication and Promotion Proposal`** с Evidence и limitations.
...
**Primary output:** `Idea Deduplication and Promotion Proposal`.
...
- название **Idea Deduplication and Promotion to DRAFT Task** и exact subject;
...
- `FR-087-01` — Функция должна выполнять только следующую ответственность: Находит duplicate/overlap, dependencies и conflicts между ideas и формирует promotion candidate в Feature Contract или DRAFT Task только для human choice.
- `FR-087-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-087-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-087-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-087-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-087-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-087`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-087-01` — Пользователь понимает назначение Idea Deduplication and Promotion to DRAFT Task без чтения implementation internals.
- `AC-087-02` — Функция создаёт ровно `Idea Deduplication and Promotion Proposal` либо честный terminal report.
- `AC-087-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-087-04` — Unknowns, exclusions и blockers видимы.
- `AC-087-05` — Human authority и downstream permissions не расширены.
- `AC-087-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Idea Deduplication and Promotion Proposal` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_087`
