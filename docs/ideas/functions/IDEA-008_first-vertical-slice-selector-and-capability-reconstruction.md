---
document_id: AOS-ATOMIC-FUNCTION-IDEA-008
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-008
function_title: "First Vertical Slice Selector and Capability Reconstruction"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-003
related_feature_families: [FTR-003]
...
**Сравнивает candidate vertical slices и выбирает для human decision минимальный end-to-end slice с прямой пользовательской ценностью, reversibility и testability.**

Основной наблюдаемый результат функции: **`First Vertical Slice Proposal`**.

`IDEA-008` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-008` family flow теряет конкретный механизм «First Vertical Slice Selector and Capability Reconstruction», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `First Vertical Slice Proposal` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: FIRST_VERTICAL_SLICE_PROPOSAL
```
...
1. Пользователь или upstream stage передаёт exact input для **First Vertical Slice Selector and Capability Reconstruction**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-008`.
4. Создаётся **`First Vertical Slice Proposal`** с Evidence и limitations.
...
**Primary output:** `First Vertical Slice Proposal`.
...
- название **First Vertical Slice Selector and Capability Reconstruction** и exact subject;
...
- `FR-008-01` — Функция должна выполнять только следующую ответственность: Сравнивает candidate vertical slices и выбирает для human decision минимальный end-to-end slice с прямой пользовательской ценностью, reversibility и testability.
- `FR-008-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-008-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-008-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-008-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-008-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-003` — Project Brief, Specification, Technical Interview and First Vertical Slice Selection](../features/FTR-003_project-brief-specification-technical-interview-and-first-vertical-slice-selection.md)
...
Каждый event содержит `function_id=IDEA-008`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-008-01` — Пользователь понимает назначение First Vertical Slice Selector and Capability Reconstruction без чтения implementation internals.
- `AC-008-02` — Функция создаёт ровно `First Vertical Slice Proposal` либо честный terminal report.
- `AC-008-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-008-04` — Unknowns, exclusions и blockers видимы.
- `AC-008-05` — Human authority и downstream permissions не расширены.
- `AC-008-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `First Vertical Slice Proposal` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_008`
