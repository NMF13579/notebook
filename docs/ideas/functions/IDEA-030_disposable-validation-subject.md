---
document_id: AOS-ATOMIC-FUNCTION-IDEA-030
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-030
function_title: "Disposable Validation Subject"
status: DRAFT
authority: NONE
...
**Создаёт disposable isolated subject для проверки candidate без загрязнения canonical workspace или подмены исходного artifact.**

Основной наблюдаемый результат функции: **`Disposable Validation Subject`**.

`IDEA-030` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-030` family flow теряет конкретный механизм «Disposable Validation Subject», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Disposable Validation Subject` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: DISPOSABLE_VALIDATION_SUBJECT
```
...
1. Пользователь или upstream stage передаёт exact input для **Disposable Validation Subject**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-030`.
4. Создаётся **`Disposable Validation Subject`** с Evidence и limitations.
...
**Primary output:** `Disposable Validation Subject`.
...
- название **Disposable Validation Subject** и exact subject;
...
- `FR-030-01` — Функция должна выполнять только следующую ответственность: Создаёт disposable isolated subject для проверки candidate без загрязнения canonical workspace или подмены исходного artifact.
- `FR-030-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-030-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-030-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-030-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-030-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-030`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-030-01` — Пользователь понимает назначение Disposable Validation Subject без чтения implementation internals.
- `AC-030-02` — Функция создаёт ровно `Disposable Validation Subject` либо честный terminal report.
- `AC-030-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-030-04` — Unknowns, exclusions и blockers видимы.
- `AC-030-05` — Human authority и downstream permissions не расширены.
- `AC-030-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Disposable Validation Subject` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_030`
