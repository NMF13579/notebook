---
document_id: AOS-ATOMIC-FUNCTION-IDEA-084
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-084
function_title: "Artifact, Decision, Blocker and Evidence Cards"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-008, FTR-012]
...
**Отображает artifacts, decisions, blockers и Evidence в единообразных cards с status, subject, provenance и доступными actions.**

Основной наблюдаемый результат функции: **`Artifact / Decision / Blocker / Evidence Card`**.

`IDEA-084` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-084` family flow теряет конкретный механизм «Artifact, Decision, Blocker and Evidence Cards», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Artifact / Decision / Blocker / Evidence Card` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ARTIFACT_DECISION_BLOCKER_EVIDENCE_CARD
```
...
1. Пользователь или upstream stage передаёт exact input для **Artifact, Decision, Blocker and Evidence Cards**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-084`.
4. Создаётся **`Artifact / Decision / Blocker / Evidence Card`** с Evidence и limitations.
...
**Primary output:** `Artifact / Decision / Blocker / Evidence Card`.
...
- название **Artifact, Decision, Blocker and Evidence Cards** и exact subject;
...
- `FR-084-01` — Функция должна выполнять только следующую ответственность: Отображает artifacts, decisions, blockers и Evidence в единообразных cards с status, subject, provenance и доступными actions.
- `FR-084-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-084-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-084-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-084-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-084-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-084`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-084-01` — Пользователь понимает назначение Artifact, Decision, Blocker and Evidence Cards без чтения implementation internals.
- `AC-084-02` — Функция создаёт ровно `Artifact / Decision / Blocker / Evidence Card` либо честный terminal report.
- `AC-084-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-084-04` — Unknowns, exclusions и blockers видимы.
- `AC-084-05` — Human authority и downstream permissions не расширены.
- `AC-084-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Artifact / Decision / Blocker / Evidence Card` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_084`
