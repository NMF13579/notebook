---
document_id: AOS-ATOMIC-FUNCTION-IDEA-018
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-018
function_title: "Action Preflight"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-009
related_feature_families: [FTR-009]
...
**Проверяет repository identity, working tree, permissions, scope, risk and prerequisites перед любым потенциальным mutation.**

Основной наблюдаемый результат функции: **`Action Preflight Report`**.

`IDEA-018` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-018` family flow теряет конкретный механизм «Action Preflight», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Action Preflight Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ACTION_PREFLIGHT_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **Action Preflight**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-018`.
4. Создаётся **`Action Preflight Report`** с Evidence и limitations.
...
**Primary output:** `Action Preflight Report`.
...
- название **Action Preflight** и exact subject;
...
- `FR-018-01` — Функция должна выполнять только следующую ответственность: Проверяет repository identity, working tree, permissions, scope, risk and prerequisites перед любым потенциальным mutation.
- `FR-018-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-018-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-018-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-018-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-018-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-009` — Repository/Action Preflight and Exact Execution Preview](../features/FTR-009_repository-action-preflight-and-exact-execution-preview.md)

Primary family layer: `Development Factory / Safety boundary`. Proposed disposition: `CANDIDATE`.
...
Каждый event содержит `function_id=IDEA-018`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-018-01` — Пользователь понимает назначение Action Preflight без чтения implementation internals.
- `AC-018-02` — Функция создаёт ровно `Action Preflight Report` либо честный terminal report.
- `AC-018-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-018-04` — Unknowns, exclusions и blockers видимы.
- `AC-018-05` — Human authority и downstream permissions не расширены.
- `AC-018-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Action Preflight Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_018`
