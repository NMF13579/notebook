---
document_id: AOS-ATOMIC-FUNCTION-IDEA-086
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-086
function_title: "Repository Ideas Workspace"
status: DRAFT
authority: NONE
...
**Создаёт repository-resident ideas workspace с одним human-readable record на идею, index, source provenance и non-authority status.**

Основной наблюдаемый результат функции: **`Repository Ideas Workspace`**.

`IDEA-086` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-086` family flow теряет конкретный механизм «Repository Ideas Workspace», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Repository Ideas Workspace` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: REPOSITORY_IDEAS_WORKSPACE
```
...
1. Пользователь или upstream stage передаёт exact input для **Repository Ideas Workspace**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-086`.
4. Создаётся **`Repository Ideas Workspace`** с Evidence и limitations.
...
**Primary output:** `Repository Ideas Workspace`.
...
- название **Repository Ideas Workspace** и exact subject;
...
- `FR-086-01` — Функция должна выполнять только следующую ответственность: Создаёт repository-resident ideas workspace с одним human-readable record на идею, index, source provenance и non-authority status.
- `FR-086-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-086-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-086-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-086-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-086-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-086`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-086-01` — Пользователь понимает назначение Repository Ideas Workspace без чтения implementation internals.
- `AC-086-02` — Функция создаёт ровно `Repository Ideas Workspace` либо честный terminal report.
- `AC-086-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-086-04` — Unknowns, exclusions и blockers видимы.
- `AC-086-05` — Human authority и downstream permissions не расширены.
- `AC-086-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Repository Ideas Workspace` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_086`
