...
**Принятое уточнение — изоляция проверяемой копии.**

- **Наблюдение — OBSERVED_AT_SNAPSHOT:** в AOS-3 проверка copied product сверяет происхождение загруженного кода в проверяемом процессе; отрицательный сценарий намеренно подставляет код исходного repository и требует отказ. Источник: `AOS3-S10` в [Reference](05_Reference.md). Прочитан код проверки, её выполнение в этом переносе — `NOT_RUN`.
- **Граница вывода:** наличие проверки не доказывает исторический инцидент, успешный текущий прогон или переносимость всего продукта.
- **Принятое правило:** при проверке отдельной копии продукта подтверждать, что проверяемый запуск использовал код именно этой копии. Проверка должна обнаруживать подмешивание product code из исходного repository или другого окружения.
- **Применимость:** проверки, заявляющие изоляцию или независимость копии продукта. Обычные тесты без такого claim не требуют отдельной проверки изоляции; язык и конкретный механизм проверки не предписываются.
- **Проверка:** положительный сценарий подтверждает происхождение product code в проверяемом запуске; отрицательный сценарий с намеренным подмешиванием кода вне копии не получает подтверждение изоляции. Самостоятельный вспомогательный процесс не доказывает происхождение кода другого запуска.
- **Статус:** `HUMAN_ACCEPTED_RULE` только для этого уточнения: текущая инструкция пользователя от 2026-09-15 «прими то что выделил для принятия» после read-only оценки кандидата E. Остальной LES-047 сохраняет `LESSON_PROPOSAL`; новые lifecycle/gates, runtime implementation и Git delivery этим решением не разрешены.
