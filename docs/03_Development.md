...
<a id="standalone-installer-plan"></a>

#### План среза: установка без Python, Git и Xcode — исполнение разрешено

**Цель.** Пользователь пишет агенту в целевом проекте «Установи AOS в этот
проект» и получает полный `aos/`, отсутствующие корневые файлы и `project/`.
Установка, запуск AOS, обновление и восстановление не требуют заранее
установленных Python, Git, Xcode, Command Line Tools или компиляторов. Агентная
среда с разрешёнными download/process/filesystem capabilities остаётся prerequisite.
Git и компиляторы самого разрабатываемого проекта — отдельные зависимости его задач.
...
Windows и macOS x86_64 требуют следующих отдельных профилей, не получают PASS
от macOS arm64. Требования принадлежат [Product](01_Product.md#aos-installed-update-decision),
...
`check_runtime` проверяет зависимости и `--help`, а не полный пользовательский
journey. `native-release.json` помечает внешний provider как `packaged: false`,
а runtime loader требует поддержанный sealed/read-only native profile.
Диагностика 2026-09-17 установила mismatch ожидаемого Xcode 26.6/SDK 26.5 и
локального Xcode 27.0/SDK 27.0 при developer rebuild. Историческая причина
отдельного FTR-015 FAIL остаётся UNKNOWN; воспроизведение timeout не доказывает
причину прошлого события. Эти observations не являются PASS нового пакета.

**Граница работы.** Сохраняются channels main/stable и dev/test, exact SHA,
ownership пользовательских файлов, explicit update, блокировка активной работы,
журнал и сверка effects. В этот срез не входят data migration, takeover чужого
unmanaged `aos/`, фоновый update, автоочистка версий, автоматическая Git-доставка,
автозаполнение проектной документации или выполнение задач проекта при установке.
Последнее обсуждённое упрощение документации остаётся отдельным кандидатом.

**Шаг 1 — связать точный исходный кандидат и новый профиль.** Перед исполнением
проверить actual branch/HEAD, tracked/untracked изменения и сохранённое Evidence;
...
проверяемые критерии без скрытого требования установить developer tools.

**Шаг 2 — получить самодостаточный локальный пакет.** Сначала на disposable
копии проверить поставку собственного CPython 3.12 с pinned dependencies,
полного portable `aos/`, launcher/manager и требуемого native provider.
Не переносить venv с абсолютной привязкой к Python машины разработчика.
Проверить relocatability, ABI, лицензии runtime/dependencies и отсутствие
ссылок на development checkout. Native сборка выполняется только у разработчика
с закреплёнными Xcode/SDK/Rust. Поддержать реальные требования read-only runtime,
provider identity и sandbox без ослабления trust-проверок ради упаковки.
Существующая ad-hoc подпись не объявляется готовой схемой публичной доставки:
проверить подходящую подпись/notarization и обычный запуск после скачивания.
Готово, когда локальный artifact запускается своим runtime из нового пути,
...
его другим commit или каналом. Private download сохраняет проверяемый login
boundary; существующая Git-аутентификация не считается автоматически пригодной
для HTTPS. Проверять издателя/доверенный источник отдельно от checksum,
контролировать redirects, лимиты и безопасную распаковку. Готово, когда exact
artifact получен без Git и исполняется только после проверки; отказ сети,
доступа, подписи или manifest не активирует непроверенные bytes.

**Шаг 4 — сделать один вход для агента и пользователя.** Подготовить постоянную
инструкцию bootstrap, которая выбирает целевой workspace и профиль, получает
проверенный готовый launcher системным HTTPS transport, затем использует его
собственный runtime. Нельзя начинать этот маршрут с `python3.12` или требовать
заранее установленный skill. Рабочий адрес инструкции появляется только после
отдельной публикации; до неё проверяется staging-equivalent, без выдуманной live URL.
Существующий skill может быть адаптером к тому же entry. Один рекомендуемый
интерфейс покрывает install/update/status/rollback; legacy entry остаётся только
для явно описанной совместимости. В agent instructions направлять запуск через
общую блокировку run/update. Установленный проект обнаруживается в новом чате
по корневому AGENTS.md без повторной настройки. Готово, когда новый чат реально
достигает этого entry, target совпадает с workspace и повторный запрос не требует
от пользователя ручного выбора Python, файлов manager или внутренних флагов.

**Шаг 5 — сохранить update, переход со старой установки и recovery.** Повторная
команда сначала проверяет pending journal и actual state той же операции.
Известную незавершённую операцию можно продолжить в прежнем scope; изменившийся
target/package, новое намерение, revoked authority или unknown effects не дают
права слепого replay. Новый запрос после reconciliation рассматривается отдельно.
Переключать согласованный набор code/runtime/launcher, не заменяя исполняемый
manager посреди операции. Проверить переход от текущего Python-managed профиля
и rollback, включая работоспособность предыдущего runtime без старого системного
Python; неподдержанный переход сообщать до переключения. Сохранить no-op для
повтора завершённого update, задачи/Evidence, AGENTS.md и пользовательские файлы.
Готово, когда crash/restart в каждом значимом этапе приводит к однозначному
active set либо объяснимому BLOCKED с сохранёнными данными.

**Шаг 6 — проверять готовность и объяснять результат.** После установки проверить
manifest, загрузку собственного runtime, чтение состояния проекта, единый launch
route и доступность компонентов заявленного сценария. Отдельно показывать:
«установлено», «базовый запуск проверен», «native-доступ проверен/требует действия».
Не представлять отсутствие provider или требуемого user selection как полную
готовность. Пользователь видит результат, версию/канал и одно конкретное действие
при проблеме; точные SHA, пути, безопасный код причины и диагностика доступны
в подробностях. Не собирать настройки/секреты пользователя ради отчёта.

**Шаг 7 — проверить полный путь и подготовить передачу.** Сначала локальная
упаковка и отрицательные случаи, затем download реального опубликованного artifact
после отдельной authority и настоящий chat journey на чистой машине/VM.
PATH masking — вспомогательная проверка, не доказательство чистой ОС. Записать
actual host/OS/arch, наличие agent capabilities, отсутствие Python/Git/Xcode/CLT
и попыток их неявной установки; зависимости не должны браться из host caches.
Проверить нужную native capability с реальным разрешённым user selection, если
он требуется контрактом. Перед доставкой провести отдельную проверку изменённых
стыков и применимую regression. Старые FAIL сохраняются: native rebuild проверяется
в закреплённой build-среде; FTR-015 требует безопасной диагностики причины,
повторный PASS не стирает исторический FAIL. Build checks и consumer checks
различаются, но обязательная недоступная проверка не становится PASS.

| Проверка плана | Критерий завершения |
|---|---|
| Первая установка | Новый чат существующего проекта без предварительного skill/runtime получает полный aos/root/project набор; произвольный код проекта не исполняется |
| Чистая среда | Install → базовый запуск → status → update работают без Python, Git, Xcode, CLT и компиляции у пользователя; проверка на реальном чистом профиле |
| Реальная готовность | Заявленные runtime/provider capabilities и их обязательные разрешения проверены отдельно; невозможная capability явно ограничивает результат |
| Каналы | main/stable по умолчанию; dev/test только явно; SHA зафиксирован; moving branch и artifact для другого SHA не меняют операцию |
| Сохранность | Existing AGENTS.md, исходники, настройки, task/Evidence и посторонние файлы сохранены; конфликт и wrong target не ведут к перезаписи |
| Переходы | Первая версия → следующая → rollback и поддержанный переход от старого managed профиля сохраняют code/runtime consistency; active/unresolved task блокирует switch |
| Прерывания | Обрыв download, process exit до/во время/после switch, потеря ack и повтор команды не создают двойной effect; unknown не переигрывается |
| Негативные случаи | Неверная подпись/hash, missing artifact, auth/rate limit, offline, недостаток места, неподдержанная OS/ABI/schema, path escape/symlink/case collision дают понятный отказ с сохранением текущей версии |
| Полный pilot | Install → реальный chat launch → stable update → interruption/reconciliation → rollback → новый чат выполнены на одном выбранном subject; fixture и реальные Human events различены |

**Материальные неизвестные до dependent шага:** источник переносимого CPython
и redistributable dependencies; совместимость sealed native runtime с layout;
доступный signing/notarization profile; адрес/хранилище immutable artifacts и
готовность пакетов для каждого main/dev SHA; HTTPS-доступ к private repository;
доступность чистого test host и настоящего agent entry. Их проверка входит в
шаги 1–4; недоступность ресурса отражается как BLOCKED соответствующего шага,
без фиктивной подписи, runtime PASS или обхода платформенных гарантий.

Ориентиры для реализации:
[GitHub: repository archives](https://docs.github.com/en/rest/repos/contents#download-a-repository-archive-tar)
описывает получение archive по ref через HTTPS, но не поставляет AOS runtime;
[Apple: notarization](https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution)
относится к подготовке macOS distribution. Конкретные версии и форматы перепроверяются
при реализации. Итог среза — проверенный consumer artifact и Evidence полного
пути; публикация, принятие результата и Git-доставка отражаются отдельно.
Сейчас все implementation/runtime/clean-host/pilot checks этого плана — `NOT_RUN`.
