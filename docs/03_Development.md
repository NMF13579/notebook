...
проверить exact ТЗ → целостный документ FTR-005 → ограничения/checks C-005 и
dependencies FTR-007. Проверяющий сверяет обязательное содержание с источниками,
...
|---|---|
| ТЗ требует сохранения и чтения записи; архитектура описывает обе части, реальный стык, хранение, отказ/восстановление, запуск и проверки; материал доступен в exact revision | FTR-006 включает соответствующие constraints/checks в C-005, FTR-007 — contribution и зависимость чтения от готового стыка; покрытая разработка продолжает общий цикл, архитектура не закрывает общий результат |
...
| ТЗ изменило требование сохранности после проверки архитектуры | Переоценить зависимую архитектуру, Brief/checks и порядок работ; прежний PASS не относится к новой revision. Незатронутые факты не переписываются |
...
| Save получил acknowledgement, но exact документ недоступен после прерывания | Handoff не подтверждён; восстановление по §10.2 до зависимого использования/повторной записи, без фиктивного PASS |
