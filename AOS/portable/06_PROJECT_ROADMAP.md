...
artifact_role: PROJECT_ROADMAP_OWNER
status: DRAFT
authority: NONE
...
|---|---|---|---|
| `D0` | Portable full documentation is harmonized, exact-bound and independently reviewable | all FTRs as design subject | current documentation gate; audit/decision separate |
...
**Goal.** Maintain one self-contained, exact-bound portable design package before implementation planning.

**Inputs.** The accepted full-project content, frozen Global Design foundation, accepted X1 decisions and all 30 feature dossiers, bound by identities in [README.md](README.md).

**Outputs.** The eleven portable Markdown owners, decision register, complete live trace graph and exact candidate manifest.
...
- exact candidate is ready for a separate independent read-only audit.

**Immediate gate.** Separate independent read-only audit with no subject correction. Human `ACCEPT | NEEDS_CHANGES | REJECT | DEFER` is a later gate on the exact candidate plus audit result. Neither gate authorizes R0/R1.
...
**Exact required human decision set.** This block is identical to the R0 set in the Decision Register:
...
| Decision | R0 outcome required |
|---|---|
| `DR-PROD-001` | one exact first user/job context |
| `DR-PROD-004` | initial R1 measurement set; numeric targets may remain unselected until baseline evidence |
| `DR-ARCH-001` | exact implementation repository and role |
| `DR-ARCH-002` | exact compatibility relation |
| `DR-ARCH-003` | initial interaction surface |
| `DR-ARCH-004` | state ownership, persistence and observable consistency outcome |
| `DR-ARCH-005` | pinned language/toolchain/dependency policy |
| `DR-POL-001` | human decision-authenticity boundary for R1 |
| `DR-POL-002` | provider/privacy/data boundary |

`DR-ARCH-006` and `DR-POL-003…004` retain R4/R5 timing. Install, Git/release and extension/domain choices retain R6–R9 timing; they are not R0 gates.
...
**Exit Evidence.** Exact human records for all nine R0 decisions; bound repository; separately authorized planning subject; focused build/test/dev loop defined; no product-success claim.
...
**Simplification.** Deliver only the minimum observable bounded-execution outcomes: authorized effects stay in scope, partial state is detectable/recoverable, intended and actual state reconcile, and terminal reporting is truthful. No full Control Plane, distributed jobs or autonomous self-heal.
...
| Domain profiles | FTR-027 | exact Medical or Design job plus separate specialist/data/rights/compliance decision; selected profile remains null until decision | DEFER |
...
| FTR-027 | R9 | separate Medical and Design profiles; `selected_domain_profile: null` | UNDECIDED |
...
|---|---|
| R0/R1 | exact set: DR-PROD-001, DR-PROD-004, DR-ARCH-001…005, DR-POL-001, DR-POL-002 |
...
1. After independent audit of the exact portable candidate, whether the phase sequence is directionally acceptable as a proposal.
2. Whether any non-X1 feature should receive an item-level disposition through its exact `DR-FTR-*` route.
3. Whether each member of the fixed nine-decision R0 set is ready to decide or should remain deferred, which blocks R0 exit.
