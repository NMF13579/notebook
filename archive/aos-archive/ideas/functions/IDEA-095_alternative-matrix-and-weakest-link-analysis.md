---
document_id: AOS-ATOMIC-FUNCTION-IDEA-095
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-095
function_title: "Alternative Matrix and Weakest-Link Analysis"
status: DRAFT
authority: NONE
...
# IDEA-095 — Alternative Matrix and Weakest-Link Analysis
...
**Сравнивает alternatives и выявляет weakest link — предположение или evidence gap, от которого сильнее всего зависит решение.**

Основной наблюдаемый результат функции: **`Alternative and Weakest-Link Matrix`**.

`IDEA-095` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-095` family flow теряет конкретный механизм «Alternative Matrix and Weakest-Link Analysis», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Alternative and Weakest-Link Matrix` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ALTERNATIVE_AND_WEAKEST_LINK_MATRIX
```
...
1. Пользователь или upstream stage передаёт exact input для **Alternative Matrix and Weakest-Link Analysis**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-095`.
4. Создаётся **`Alternative and Weakest-Link Matrix`** с Evidence и limitations.
...
**Primary output:** `Alternative and Weakest-Link Matrix`.
...
- название **Alternative Matrix and Weakest-Link Analysis** и exact subject;
...
- `FR-095-01` — Функция должна выполнять только следующую ответственность: Сравнивает alternatives и выявляет weakest link — предположение или evidence gap, от которого сильнее всего зависит решение.
- `FR-095-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-095-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-095-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-095-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-095-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-095`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-095-01` — Пользователь понимает назначение Alternative Matrix and Weakest-Link Analysis без чтения implementation internals.
- `AC-095-02` — Функция создаёт ровно `Alternative and Weakest-Link Matrix` либо честный terminal report.
- `AC-095-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-095-04` — Unknowns, exclusions и blockers видимы.
- `AC-095-05` — Human authority и downstream permissions не расширены.
- `AC-095-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `Alternative and Weakest-Link Matrix` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_095`
