---
document_id: AOS-ATOMIC-FUNCTION-IDEA-043
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-043
function_title: "Safe Apply and Manual Transfer"
status: DRAFT
authority: NONE
...
**Применяет заранее просмотренный install/scaffold plan или формирует manual-transfer package с idempotency и сохранением user-owned files.**

Основной наблюдаемый результат функции: **`Safe Apply / Manual Transfer Result`**.

`IDEA-043` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-043` family flow теряет конкретный механизм «Safe Apply and Manual Transfer», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Safe Apply / Manual Transfer Result` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: SAFE_APPLY_MANUAL_TRANSFER_RESULT
```
...
1. Пользователь или upstream stage передаёт exact input для **Safe Apply and Manual Transfer**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-043`.
4. Создаётся **`Safe Apply / Manual Transfer Result`** с Evidence и limitations.
...
**Primary output:** `Safe Apply / Manual Transfer Result`.
...
- название **Safe Apply and Manual Transfer** и exact subject;
...
- `FR-043-01` — Функция должна выполнять только следующую ответственность: Применяет заранее просмотренный install/scaffold plan или формирует manual-transfer package с idempotency и сохранением user-owned files.
- `FR-043-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-043-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-043-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-043-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-043-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-043`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-043-01` — Пользователь понимает назначение Safe Apply and Manual Transfer без чтения implementation internals.
- `AC-043-02` — Функция создаёт ровно `Safe Apply / Manual Transfer Result` либо честный terminal report.
- `AC-043-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-043-04` — Unknowns, exclusions и blockers видимы.
- `AC-043-05` — Human authority и downstream permissions не расширены.
- `AC-043-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Safe Apply / Manual Transfer Result` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_043`
