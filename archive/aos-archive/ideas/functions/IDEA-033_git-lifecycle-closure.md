---
document_id: AOS-ATOMIC-FUNCTION-IDEA-033
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-033
function_title: "Git Lifecycle Closure"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-015
related_feature_families: [FTR-015]
...
**Разделяет Commit, Push, PR create, Merge и Release, проверяя repository/remote/candidate identity перед каждой Git lifecycle operation.**

Основной наблюдаемый результат функции: **`Git Lifecycle Closure Record`**.

`IDEA-033` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-033` family flow теряет конкретный механизм «Git Lifecycle Closure», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Git Lifecycle Closure Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: GIT_LIFECYCLE_CLOSURE_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Git Lifecycle Closure**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-033`.
4. Создаётся **`Git Lifecycle Closure Record`** с Evidence и limitations.
...
**Primary output:** `Git Lifecycle Closure Record`.
...
- название **Git Lifecycle Closure** и exact subject;
...
- `FR-033-01` — Функция должна выполнять только следующую ответственность: Разделяет Commit, Push, PR create, Merge и Release, проверяя repository/remote/candidate identity перед каждой Git lifecycle operation.
- `FR-033-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-033-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-033-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-033-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-033-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-015` — Git Lifecycle Closure, Remote State and Separate Commit/Push/Merge/Release Authorization](../features/FTR-015_git-lifecycle-closure-remote-state-and-separate-commit-push-merge-release-authorization.md)

Primary family layer: `Development Factory / Delivery boundary`. Proposed disposition: `CANDIDATE`.
...
Каждый event содержит `function_id=IDEA-033`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-033-01` — Пользователь понимает назначение Git Lifecycle Closure без чтения implementation internals.
- `AC-033-02` — Функция создаёт ровно `Git Lifecycle Closure Record` либо честный terminal report.
- `AC-033-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-033-04` — Unknowns, exclusions и blockers видимы.
- `AC-033-05` — Human authority и downstream permissions не расширены.
- `AC-033-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Git Lifecycle Closure Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_033`
