---
document_id: AOS-ATOMIC-FUNCTION-IDEA-010
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-010
function_title: "Architecture Assistant"
status: DRAFT
authority: NONE
...
**Помогает структурировать architecture problem, boundaries, quality attributes и candidate directions, сохраняя решения за человеком.**

Основной наблюдаемый результат функции: **`Architecture Analysis Draft`**.

`IDEA-010` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-010` family flow теряет конкретный механизм «Architecture Assistant», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Architecture Analysis Draft` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ARCHITECTURE_ANALYSIS_DRAFT
```
...
1. Пользователь или upstream stage передаёт exact input для **Architecture Assistant**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-010`.
4. Создаётся **`Architecture Analysis Draft`** с Evidence и limitations.
...
**Primary output:** `Architecture Analysis Draft`.
...
- название **Architecture Assistant** и exact subject;
...
- `FR-010-01` — Функция должна выполнять только следующую ответственность: Помогает структурировать architecture problem, boundaries, quality attributes и candidate directions, сохраняя решения за человеком.
- `FR-010-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-010-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-010-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-010-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-010-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-010`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-010-01` — Пользователь понимает назначение Architecture Assistant без чтения implementation internals.
- `AC-010-02` — Функция создаёт ровно `Architecture Analysis Draft` либо честный terminal report.
- `AC-010-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-010-04` — Unknowns, exclusions и blockers видимы.
- `AC-010-05` — Human authority и downstream permissions не расширены.
- `AC-010-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Architecture Analysis Draft` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_010`
