---
document_id: AOS-ATOMIC-FUNCTION-IDEA-032
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-032
function_title: "Recovery, Resume and Rollback"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-014]
...
**Определяет безопасный путь recovery, resume или rollback после interruption/failure, отделяя assessment от отдельно authorized corrective action.**

Основной наблюдаемый результат функции: **`Recovery / Resume / Rollback Plan`**.

`IDEA-032` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-032` family flow теряет конкретный механизм «Recovery, Resume and Rollback», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Recovery / Resume / Rollback Plan` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: RECOVERY_RESUME_ROLLBACK_PLAN
```
...
1. Пользователь или upstream stage передаёт exact input для **Recovery, Resume and Rollback**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-032`.
4. Создаётся **`Recovery / Resume / Rollback Plan`** с Evidence и limitations.
...
**Primary output:** `Recovery / Resume / Rollback Plan`.
...
- название **Recovery, Resume and Rollback** и exact subject;
...
- `FR-032-01` — Функция должна выполнять только следующую ответственность: Определяет безопасный путь recovery, resume или rollback после interruption/failure, отделяя assessment от отдельно authorized corrective action.
- `FR-032-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-032-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-032-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-032-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-032-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-032`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-032-01` — Пользователь понимает назначение Recovery, Resume and Rollback без чтения implementation internals.
- `AC-032-02` — Функция создаёт ровно `Recovery / Resume / Rollback Plan` либо честный terminal report.
- `AC-032-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-032-04` — Unknowns, exclusions и blockers видимы.
- `AC-032-05` — Human authority и downstream permissions не расширены.
- `AC-032-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Recovery / Resume / Rollback Plan` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_032`
