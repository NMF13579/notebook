---
document_id: AOS-ATOMIC-FUNCTION-IDEA-028
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-028
function_title: "Human Decision Record and False-PASS Wording Guard"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-012, FTR-021]
...
**Записывает реальное human decision для exact subject и предотвращает формулировки, в которых PASS, Evidence или agent output имитируют approval.**

Основной наблюдаемый результат функции: **`Human Decision Record`**.

`IDEA-028` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-028` family flow теряет конкретный механизм «Human Decision Record and False-PASS Wording Guard», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Human Decision Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: HUMAN_DECISION_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Human Decision Record and False-PASS Wording Guard**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-028`.
4. Создаётся **`Human Decision Record`** с Evidence и limitations.
...
**Primary output:** `Human Decision Record`.
...
- название **Human Decision Record and False-PASS Wording Guard** и exact subject;
...
- `FR-028-01` — Функция должна выполнять только следующую ответственность: Записывает реальное human decision для exact subject и предотвращает формулировки, в которых PASS, Evidence или agent output имитируют approval.
- `FR-028-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-028-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-028-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-028-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-028-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-028`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-028-01` — Пользователь понимает назначение Human Decision Record and False-PASS Wording Guard без чтения implementation internals.
- `AC-028-02` — Функция создаёт ровно `Human Decision Record` либо честный terminal report.
- `AC-028-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-028-04` — Unknowns, exclusions и blockers видимы.
- `AC-028-05` — Human authority и downstream permissions не расширены.
- `AC-028-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Human Decision Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_028`
