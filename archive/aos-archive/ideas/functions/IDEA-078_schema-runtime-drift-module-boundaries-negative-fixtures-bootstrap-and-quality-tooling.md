---
document_id: AOS-ATOMIC-FUNCTION-IDEA-078
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-078
function_title: "Schema / Runtime Drift, Module Boundaries, Negative Fixtures, Bootstrap and Quality Tooling"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-023
related_feature_families: [FTR-023, FTR-030]
...
**Проверяет schema/runtime drift, module boundaries, negative fixtures, bootstrap and quality tooling после стабилизации базовых contracts.**

Основной наблюдаемый результат функции: **`Contract and Boundary Test Report`**.

`IDEA-078` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-078` family flow теряет конкретный механизм «Schema / Runtime Drift, Module Boundaries, Negative Fixtures, Bootstrap and Quality Tooling», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Contract and Boundary Test Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: CONTRACT_AND_BOUNDARY_TEST_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **Schema / Runtime Drift, Module Boundaries, Negative Fixtures, Bootstrap and Quality Tooling**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-078`.
4. Создаётся **`Contract and Boundary Test Report`** с Evidence и limitations.
...
**Primary output:** `Contract and Boundary Test Report`.
...
- название **Schema / Runtime Drift, Module Boundaries, Negative Fixtures, Bootstrap and Quality Tooling** и exact subject;
...
- `FR-078-01` — Функция должна выполнять только следующую ответственность: Проверяет schema/runtime drift, module boundaries, negative fixtures, bootstrap and quality tooling после стабилизации базовых contracts.
- `FR-078-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-078-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-078-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-078-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-078-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-023` — Advisory CI, CI Smoke, Safety Regression Fixtures and Quality Gates](../features/FTR-023_advisory-ci-ci-smoke-safety-regression-fixtures-and-quality-gates.md)
...
Primary family layer: `Development Factory`. Proposed disposition: `CANDIDATE_AFTER_BASELINE`.
...
Каждый event содержит `function_id=IDEA-078`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-078-01` — Пользователь понимает назначение Schema / Runtime Drift, Module Boundaries, Negative Fixtures, Bootstrap and Quality Tooling без чтения implementation internals.
- `AC-078-02` — Функция создаёт ровно `Contract and Boundary Test Report` либо честный terminal report.
- `AC-078-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-078-04` — Unknowns, exclusions и blockers видимы.
- `AC-078-05` — Human authority и downstream permissions не расширены.
- `AC-078-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Contract and Boundary Test Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_078`
