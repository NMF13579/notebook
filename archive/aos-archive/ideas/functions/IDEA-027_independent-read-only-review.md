---
document_id: AOS-ATOMIC-FUNCTION-IDEA-027
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-027
function_title: "Independent Read-only Review"
status: DRAFT
authority: NONE
...
**Проводит независимый read-only review frozen candidate без исправления subject, фиксируя findings и limitations.**

Основной наблюдаемый результат функции: **`Independent Review Report`**.

`IDEA-027` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-027` family flow теряет конкретный механизм «Independent Read-only Review», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Independent Review Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: INDEPENDENT_REVIEW_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **Independent Read-only Review**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-027`.
4. Создаётся **`Independent Review Report`** с Evidence и limitations.
...
**Primary output:** `Independent Review Report`.
...
- название **Independent Read-only Review** и exact subject;
...
- `FR-027-01` — Функция должна выполнять только следующую ответственность: Проводит независимый read-only review frozen candidate без исправления subject, фиксируя findings и limitations.
- `FR-027-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-027-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-027-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-027-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-027-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-027`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-027-01` — Пользователь понимает назначение Independent Read-only Review без чтения implementation internals.
- `AC-027-02` — Функция создаёт ровно `Independent Review Report` либо честный terminal report.
- `AC-027-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-027-04` — Unknowns, exclusions и blockers видимы.
- `AC-027-05` — Human authority и downstream permissions не расширены.
- `AC-027-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Independent Review Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_027`
