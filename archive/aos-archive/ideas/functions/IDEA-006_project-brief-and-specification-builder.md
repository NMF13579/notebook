---
document_id: AOS-ATOMIC-FUNCTION-IDEA-006
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-006
function_title: "Project Brief and Specification Builder"
status: DRAFT
authority: NONE
...
**Собирает Intent и подтверждённые факты в самодостаточный Project Brief и Product/Feature Specification с stable requirement IDs, journeys, constraints и non-goals.**

Основной наблюдаемый результат функции: **`Project Brief / Specification`**.

`IDEA-006` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-006` family flow теряет конкретный механизм «Project Brief and Specification Builder», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Project Brief / Specification` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: PROJECT_BRIEF_SPECIFICATION
```
...
1. Пользователь или upstream stage передаёт exact input для **Project Brief and Specification Builder**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-006`.
4. Создаётся **`Project Brief / Specification`** с Evidence и limitations.
...
**Primary output:** `Project Brief / Specification`.
...
- название **Project Brief and Specification Builder** и exact subject;
...
- `FR-006-01` — Функция должна выполнять только следующую ответственность: Собирает Intent и подтверждённые факты в самодостаточный Project Brief и Product/Feature Specification с stable requirement IDs, journeys, constraints и non-goals.
- `FR-006-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-006-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-006-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-006-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-006-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-006`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-006-01` — Пользователь понимает назначение Project Brief and Specification Builder без чтения implementation internals.
- `AC-006-02` — Функция создаёт ровно `Project Brief / Specification` либо честный terminal report.
- `AC-006-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-006-04` — Unknowns, exclusions и blockers видимы.
- `AC-006-05` — Human authority и downstream permissions не расширены.
- `AC-006-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Project Brief / Specification` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_006`
