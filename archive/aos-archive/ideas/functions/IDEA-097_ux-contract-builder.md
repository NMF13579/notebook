---
document_id: AOS-ATOMIC-FUNCTION-IDEA-097
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-097
function_title: "UX Contract Builder"
status: DRAFT
authority: NONE
...
**Формирует formal UX Contract со screens, flows, states, actions, edge cases, accessibility, non-goals и requirement traceability.**

Основной наблюдаемый результат функции: **`UX Contract`**.

`IDEA-097` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-097` family flow теряет конкретный механизм «UX Contract Builder», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `UX Contract` и прямо связанные derived views.
...
1. Пользователь или upstream stage передаёт exact input для **UX Contract Builder**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-097`.
4. Создаётся **`UX Contract`** с Evidence и limitations.
...
- название **UX Contract Builder** и exact subject;
...
- `FR-097-01` — Функция должна выполнять только следующую ответственность: Формирует formal UX Contract со screens, flows, states, actions, edge cases, accessibility, non-goals и requirement traceability.
- `FR-097-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-097-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-097-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-097-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-097-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-097`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-097-01` — Пользователь понимает назначение UX Contract Builder без чтения implementation internals.
- `AC-097-02` — Функция создаёт ровно `UX Contract` либо честный terminal report.
- `AC-097-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-097-04` — Unknowns, exclusions и blockers видимы.
- `AC-097-05` — Human authority и downstream permissions не расширены.
- `AC-097-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `UX Contract` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_097`
