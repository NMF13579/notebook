---
document_id: AOS-ATOMIC-FUNCTION-IDEA-080
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-080
function_title: "Ready-Spec vs Build-from-Scratch Entry"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-001, FTR-003]
...
**Явно разделяет вход READY_SPEC и BUILD_FROM_SCRATCH, направляя их в разные documentation flows без потери исходного контекста.**

Основной наблюдаемый результат функции: **`Specification Entry Route`**.

`IDEA-080` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-080` family flow теряет конкретный механизм «Ready-Spec vs Build-from-Scratch Entry», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Specification Entry Route` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: SPECIFICATION_ENTRY_ROUTE
```
...
1. Пользователь или upstream stage передаёт exact input для **Ready-Spec vs Build-from-Scratch Entry**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-080`.
4. Создаётся **`Specification Entry Route`** с Evidence и limitations.
...
**Primary output:** `Specification Entry Route`.
...
- название **Ready-Spec vs Build-from-Scratch Entry** и exact subject;
...
- `FR-080-01` — Функция должна выполнять только следующую ответственность: Явно разделяет вход READY_SPEC и BUILD_FROM_SCRATCH, направляя их в разные documentation flows без потери исходного контекста.
- `FR-080-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-080-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-080-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-080-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-080-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-080`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-080-01` — Пользователь понимает назначение Ready-Spec vs Build-from-Scratch Entry без чтения implementation internals.
- `AC-080-02` — Функция создаёт ровно `Specification Entry Route` либо честный terminal report.
- `AC-080-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-080-04` — Unknowns, exclusions и blockers видимы.
- `AC-080-05` — Human authority и downstream permissions не расширены.
- `AC-080-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Specification Entry Route` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_080`
