---
document_id: AOS-ATOMIC-FUNCTION-IDEA-098
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-098
function_title: "UX Structural Validator"
status: DRAFT
authority: NONE
...
# IDEA-098 — UX Structural Validator
...
**Проверяет UX Contract на vocabulary, required states, navigation, recovery, traceability и authority-boundary defects.**

Основной наблюдаемый результат функции: **`UX Contract Validation Report`**.

`IDEA-098` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-098` family flow теряет конкретный механизм «UX Structural Validator», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `UX Contract Validation Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: UX_CONTRACT_VALIDATION_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **UX Structural Validator**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-098`.
4. Создаётся **`UX Contract Validation Report`** с Evidence и limitations.
...
**Primary output:** `UX Contract Validation Report`.
...
- название **UX Structural Validator** и exact subject;
...
- `FR-098-01` — Функция должна выполнять только следующую ответственность: Проверяет UX Contract на vocabulary, required states, navigation, recovery, traceability и authority-boundary defects.
- `FR-098-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-098-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-098-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-098-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-098-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-098`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-098-01` — Пользователь понимает назначение UX Structural Validator без чтения implementation internals.
- `AC-098-02` — Функция создаёт ровно `UX Contract Validation Report` либо честный terminal report.
- `AC-098-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-098-04` — Unknowns, exclusions и blockers видимы.
- `AC-098-05` — Human authority и downstream permissions не расширены.
- `AC-098-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `UX Contract Validation Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_098`
