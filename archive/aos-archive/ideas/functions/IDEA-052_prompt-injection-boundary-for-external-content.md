---
document_id: AOS-ATOMIC-FUNCTION-IDEA-052
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-052
function_title: "Prompt Injection Boundary for External Content"
status: DRAFT
authority: NONE
...
**Изолирует инструкции из external content как untrusted data и предотвращает prompt injection, authority spoofing и hidden tool escalation.**

Основной наблюдаемый результат функции: **`External Content Safety Assessment`**.

`IDEA-052` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-052` family flow теряет конкретный механизм «Prompt Injection Boundary for External Content», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `External Content Safety Assessment` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: EXTERNAL_CONTENT_SAFETY_ASSESSMENT
```
...
1. Пользователь или upstream stage передаёт exact input для **Prompt Injection Boundary for External Content**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-052`.
4. Создаётся **`External Content Safety Assessment`** с Evidence и limitations.
...
**Primary output:** `External Content Safety Assessment`.
...
- название **Prompt Injection Boundary for External Content** и exact subject;
...
- `FR-052-01` — Функция должна выполнять только следующую ответственность: Изолирует инструкции из external content как untrusted data и предотвращает prompt injection, authority spoofing и hidden tool escalation.
- `FR-052-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-052-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-052-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-052-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-052-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-052`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-052-01` — Пользователь понимает назначение Prompt Injection Boundary for External Content без чтения implementation internals.
- `AC-052-02` — Функция создаёт ровно `External Content Safety Assessment` либо честный terminal report.
- `AC-052-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-052-04` — Unknowns, exclusions и blockers видимы.
- `AC-052-05` — Human authority и downstream permissions не расширены.
- `AC-052-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `External Content Safety Assessment` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_052`
