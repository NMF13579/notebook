---
document_id: AOS-ATOMIC-FUNCTION-IDEA-024
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-024
function_title: "Doctor and Self-Test"
status: DRAFT
authority: NONE
...
**Проверяет install/runtime health, configuration, critical contracts и minimal self-test path, не выдавая historical PASS за current readiness.**

Основной наблюдаемый результат функции: **`Doctor / Self-Test Report`**.

`IDEA-024` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-024` family flow теряет конкретный механизм «Doctor and Self-Test», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Doctor / Self-Test Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: DOCTOR_SELF_TEST_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **Doctor and Self-Test**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-024`.
4. Создаётся **`Doctor / Self-Test Report`** с Evidence и limitations.
...
**Primary output:** `Doctor / Self-Test Report`.
...
- название **Doctor and Self-Test** и exact subject;
...
- `FR-024-01` — Функция должна выполнять только следующую ответственность: Проверяет install/runtime health, configuration, critical contracts и minimal self-test path, не выдавая historical PASS за current readiness.
- `FR-024-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-024-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-024-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-024-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-024-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-024`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-024-01` — Пользователь понимает назначение Doctor and Self-Test без чтения implementation internals.
- `AC-024-02` — Функция создаёт ровно `Doctor / Self-Test Report` либо честный terminal report.
- `AC-024-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-024-04` — Unknowns, exclusions и blockers видимы.
- `AC-024-05` — Human authority и downstream permissions не расширены.
- `AC-024-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Doctor / Self-Test Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_024`
