---
document_id: AOS-ATOMIC-FUNCTION-IDEA-070
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-070
function_title: "Template Export, Prompt Packs, Capability Modules, Cross-Repo Context, Localization and Policy Overlays"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-029]
...
**Экспортирует templates, prompt packs и capability modules, поддерживая cross-repo context, localization и policy overlays с provenance и compatibility.**

Основной наблюдаемый результат функции: **`Template / Prompt / Overlay Package`**.

`IDEA-070` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-070` family flow теряет конкретный механизм «Template Export, Prompt Packs, Capability Modules, Cross-Repo Context, Localization and Policy Overlays», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Template / Prompt / Overlay Package` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: TEMPLATE_PROMPT_OVERLAY_PACKAGE
```
...
1. Пользователь или upstream stage передаёт exact input для **Template Export, Prompt Packs, Capability Modules, Cross-Repo Context, Localization and Policy Overlays**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-070`.
4. Создаётся **`Template / Prompt / Overlay Package`** с Evidence и limitations.
...
**Primary output:** `Template / Prompt / Overlay Package`.
...
- название **Template Export, Prompt Packs, Capability Modules, Cross-Repo Context, Localization and Policy Overlays** и exact subject;
...
- `FR-070-01` — Функция должна выполнять только следующую ответственность: Экспортирует templates, prompt packs и capability modules, поддерживая cross-repo context, localization и policy overlays с provenance и compatibility.
- `FR-070-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-070-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-070-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-070-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-070-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-070`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-070-01` — Пользователь понимает назначение Template Export, Prompt Packs, Capability Modules, Cross-Repo Context, Localization and Policy Overlays без чтения implementation internals.
- `AC-070-02` — Функция создаёт ровно `Template / Prompt / Overlay Package` либо честный terminal report.
- `AC-070-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-070-04` — Unknowns, exclusions и blockers видимы.
- `AC-070-05` — Human authority и downstream permissions не расширены.
- `AC-070-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Template / Prompt / Overlay Package` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_070`
