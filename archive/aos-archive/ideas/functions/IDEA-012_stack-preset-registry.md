---
document_id: AOS-ATOMIC-FUNCTION-IDEA-012
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-012
function_title: "Stack Preset Registry"
status: DRAFT
authority: NONE
...
**Хранит повторно используемые stack presets как advisory references с условиями применимости, ограничениями и version provenance.**

Основной наблюдаемый результат функции: **`Stack Preset Record`**.

`IDEA-012` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-012` family flow теряет конкретный механизм «Stack Preset Registry», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Stack Preset Record` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: STACK_PRESET_RECORD
```
...
1. Пользователь или upstream stage передаёт exact input для **Stack Preset Registry**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-012`.
4. Создаётся **`Stack Preset Record`** с Evidence и limitations.
...
**Primary output:** `Stack Preset Record`.
...
- название **Stack Preset Registry** и exact subject;
...
- `FR-012-01` — Функция должна выполнять только следующую ответственность: Хранит повторно используемые stack presets как advisory references с условиями применимости, ограничениями и version provenance.
- `FR-012-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-012-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-012-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-012-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-012-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-012`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-012-01` — Пользователь понимает назначение Stack Preset Registry без чтения implementation internals.
- `AC-012-02` — Функция создаёт ровно `Stack Preset Record` либо честный terminal report.
- `AC-012-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-012-04` — Unknowns, exclusions и blockers видимы.
- `AC-012-05` — Human authority и downstream permissions не расширены.
- `AC-012-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Stack Preset Record` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_012`
