---
document_id: AOS-ATOMIC-FUNCTION-IDEA-076
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-076
function_title: "Task Brief Compiler and Report Builder"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-006, FTR-030]
...
**Компилирует human-readable Task Brief и Stage Report из structured fields, не добавляя authority и не скрывая unknowns.**

Основной наблюдаемый результат функции: **`Compiled Task Brief / Stage Report`**.

`IDEA-076` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-076` family flow теряет конкретный механизм «Task Brief Compiler and Report Builder», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Compiled Task Brief / Stage Report` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: COMPILED_TASK_BRIEF_STAGE_REPORT
```
...
1. Пользователь или upstream stage передаёт exact input для **Task Brief Compiler and Report Builder**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-076`.
4. Создаётся **`Compiled Task Brief / Stage Report`** с Evidence и limitations.
...
**Primary output:** `Compiled Task Brief / Stage Report`.
...
- название **Task Brief Compiler and Report Builder** и exact subject;
...
- `FR-076-01` — Функция должна выполнять только следующую ответственность: Компилирует human-readable Task Brief и Stage Report из structured fields, не добавляя authority и не скрывая unknowns.
- `FR-076-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-076-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-076-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-076-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-076-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-076`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-076-01` — Пользователь понимает назначение Task Brief Compiler and Report Builder без чтения implementation internals.
- `AC-076-02` — Функция создаёт ровно `Compiled Task Brief / Stage Report` либо честный terminal report.
- `AC-076-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-076-04` — Unknowns, exclusions и blockers видимы.
- `AC-076-05` — Human authority и downstream permissions не расширены.
- `AC-076-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Compiled Task Brief / Stage Report` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_076`
