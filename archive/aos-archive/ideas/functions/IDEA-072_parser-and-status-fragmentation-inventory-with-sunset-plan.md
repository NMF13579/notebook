---
document_id: AOS-ATOMIC-FUNCTION-IDEA-072
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-072
function_title: "Parser and Status Fragmentation Inventory with Sunset Plan"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-030
related_feature_families: [FTR-030]
...
**Инвентаризирует разрозненные parsers/status contracts и формирует sunset/migration plan к единому owner без premature rewrite.**

Основной наблюдаемый результат функции: **`Parser and Status Sunset Plan`**.

`IDEA-072` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-072` family flow теряет конкретный механизм «Parser and Status Fragmentation Inventory with Sunset Plan», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Parser and Status Sunset Plan` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: PARSER_AND_STATUS_SUNSET_PLAN
```
...
1. Пользователь или upstream stage передаёт exact input для **Parser and Status Fragmentation Inventory with Sunset Plan**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-072`.
4. Создаётся **`Parser and Status Sunset Plan`** с Evidence и limitations.
...
**Primary output:** `Parser and Status Sunset Plan`.
...
- название **Parser and Status Fragmentation Inventory with Sunset Plan** и exact subject;
...
- `FR-072-01` — Функция должна выполнять только следующую ответственность: Инвентаризирует разрозненные parsers/status contracts и формирует sunset/migration plan к единому owner без premature rewrite.
- `FR-072-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-072-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-072-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-072-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-072-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Primary family layer: `Development Factory internal`. Proposed disposition: `INTERNAL_RESEARCH_AFTER_CONTRACTS`.
...
Каждый event содержит `function_id=IDEA-072`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-072-01` — Пользователь понимает назначение Parser and Status Fragmentation Inventory with Sunset Plan без чтения implementation internals.
- `AC-072-02` — Функция создаёт ровно `Parser and Status Sunset Plan` либо честный terminal report.
- `AC-072-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-072-04` — Unknowns, exclusions и blockers видимы.
- `AC-072-05` — Human authority и downstream permissions не расширены.
- `AC-072-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Parser and Status Sunset Plan` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_072`
