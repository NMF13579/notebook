---
document_id: AOS-ATOMIC-FUNCTION-IDEA-044
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-044
function_title: "Safe Update and Uninstall"
status: DRAFT
authority: NONE
...
**Безопасно обновляет или удаляет AOS-managed assets по manifest, сохраняя project/user state и выдавая recovery plan при partial result.**

Основной наблюдаемый результат функции: **`Update / Uninstall Result`**.

`IDEA-044` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-044` family flow теряет конкретный механизм «Safe Update and Uninstall», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Update / Uninstall Result` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: UPDATE_UNINSTALL_RESULT
```
...
1. Пользователь или upstream stage передаёт exact input для **Safe Update and Uninstall**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-044`.
4. Создаётся **`Update / Uninstall Result`** с Evidence и limitations.
...
**Primary output:** `Update / Uninstall Result`.
...
- название **Safe Update and Uninstall** и exact subject;
...
- `FR-044-01` — Функция должна выполнять только следующую ответственность: Безопасно обновляет или удаляет AOS-managed assets по manifest, сохраняя project/user state и выдавая recovery plan при partial result.
- `FR-044-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-044-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-044-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-044-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-044-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-044`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-044-01` — Пользователь понимает назначение Safe Update and Uninstall без чтения implementation internals.
- `AC-044-02` — Функция создаёт ровно `Update / Uninstall Result` либо честный terminal report.
- `AC-044-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-044-04` — Unknowns, exclusions и blockers видимы.
- `AC-044-05` — Human authority и downstream permissions не расширены.
- `AC-044-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Update / Uninstall Result` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_044`
