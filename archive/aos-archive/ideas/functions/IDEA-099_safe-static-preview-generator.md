---
document_id: AOS-ATOMIC-FUNCTION-IDEA-099
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-099
function_title: "Safe Static Preview Generator"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-034
related_feature_families: [FTR-034]
human_review_required: true
---

# IDEA-099 — Safe Static Preview Generator
...
**Генерирует visualization-only static preview без JavaScript, forms, active controls, network и external assets, явно маркируя его как preview, not approval.**

Основной наблюдаемый результат функции: **`Safe Static UX Preview`**.

`IDEA-099` — Новый working ID, введённый только в R2 для атомарного разделения FTR-033/FTR-034; это `PROPOSAL`, не historical fact.
...
**Atomic focus:** без `IDEA-099` family flow теряет конкретный механизм «Safe Static Preview Generator», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Safe Static UX Preview` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: SAFE_STATIC_UX_PREVIEW
```
...
1. Пользователь или upstream stage передаёт exact input для **Safe Static Preview Generator**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-099`.
4. Создаётся **`Safe Static UX Preview`** с Evidence и limitations.
...
**Primary output:** `Safe Static UX Preview`.
...
- название **Safe Static Preview Generator** и exact subject;
...
- `FR-099-01` — Функция должна выполнять только следующую ответственность: Генерирует visualization-only static preview без JavaScript, forms, active controls, network и external assets, явно маркируя его как preview, not approval.
- `FR-099-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-099-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-099-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-099-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-099-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-034` — UX Contract, Structural Validation, Safe Static Preview and Visual-Direction Snapshot](../features/FTR-034_ux-contract-structural-validation-safe-static-preview-and-visual-direction-snapshot.md)

Primary family layer: `Product Runtime support / UX review boundary`. Proposed disposition: `CANDIDATE_SUPPORT`.
...
Каждый event содержит `function_id=IDEA-099`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-099-01` — Пользователь понимает назначение Safe Static Preview Generator без чтения implementation internals.
- `AC-099-02` — Функция создаёт ровно `Safe Static UX Preview` либо честный terminal report.
- `AC-099-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-099-04` — Unknowns, exclusions и blockers видимы.
- `AC-099-05` — Human authority и downstream permissions не расширены.
- `AC-099-06` — Failure behavior соответствует report + stop.

## 20. Test design — `NOT_RUN`

### Positive tests

- При валидных входах создаётся `Safe Static UX Preview` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_099`
