---
document_id: AOS-ATOMIC-FUNCTION-IDEA-013
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-013
function_title: "Pattern / Solution Library and Fit Matrix"
status: DRAFT
authority: NONE
...
related_feature_families: [FTR-022]
...
**Поддерживает библиотеку solution patterns и fit matrix, чтобы предлагать проверенные подходы без автоматического превращения pattern в обязательную architecture.**

Основной наблюдаемый результат функции: **`Pattern Record and Fit Matrix`**.

`IDEA-013` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-013` family flow теряет конкретный механизм «Pattern / Solution Library and Fit Matrix», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Pattern Record and Fit Matrix` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: PATTERN_RECORD_AND_FIT_MATRIX
```
...
1. Пользователь или upstream stage передаёт exact input для **Pattern / Solution Library and Fit Matrix**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-013`.
4. Создаётся **`Pattern Record and Fit Matrix`** с Evidence и limitations.
...
**Primary output:** `Pattern Record and Fit Matrix`.
...
- название **Pattern / Solution Library and Fit Matrix** и exact subject;
...
- `FR-013-01` — Функция должна выполнять только следующую ответственность: Поддерживает библиотеку solution patterns и fit matrix, чтобы предлагать проверенные подходы без автоматического превращения pattern в обязательную architecture.
- `FR-013-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-013-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-013-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-013-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-013-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-013`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-013-01` — Пользователь понимает назначение Pattern / Solution Library and Fit Matrix без чтения implementation internals.
- `AC-013-02` — Функция создаёт ровно `Pattern Record and Fit Matrix` либо честный terminal report.
- `AC-013-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-013-04` — Unknowns, exclusions и blockers видимы.
- `AC-013-05` — Human authority и downstream permissions не расширены.
- `AC-013-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Pattern Record and Fit Matrix` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_013`
