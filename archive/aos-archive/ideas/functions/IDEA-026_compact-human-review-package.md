---
document_id: AOS-ATOMIC-FUNCTION-IDEA-026
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-026
function_title: "Compact Human Review Package"
status: DRAFT
authority: NONE
...
**Сжимает технические результаты в понятный человеку review package: что изменено, что проверено, findings, risks, unknowns и требуемое решение.**

Основной наблюдаемый результат функции: **`Human Review Package`**.

`IDEA-026` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-026` family flow теряет конкретный механизм «Compact Human Review Package», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Human Review Package` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: HUMAN_REVIEW_PACKAGE
```
...
1. Пользователь или upstream stage передаёт exact input для **Compact Human Review Package**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-026`.
4. Создаётся **`Human Review Package`** с Evidence и limitations.
...
**Primary output:** `Human Review Package`.
...
- название **Compact Human Review Package** и exact subject;
...
- `FR-026-01` — Функция должна выполнять только следующую ответственность: Сжимает технические результаты в понятный человеку review package: что изменено, что проверено, findings, risks, unknowns и требуемое решение.
- `FR-026-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-026-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-026-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-026-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-026-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-026`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-026-01` — Пользователь понимает назначение Compact Human Review Package без чтения implementation internals.
- `AC-026-02` — Функция создаёт ровно `Human Review Package` либо честный terminal report.
- `AC-026-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-026-04` — Unknowns, exclusions и blockers видимы.
- `AC-026-05` — Human authority и downstream permissions не расширены.
- `AC-026-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Human Review Package` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_026`
