---
document_id: AOS-ATOMIC-FUNCTION-IDEA-058
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-058
function_title: "Advanced Governance Gates"
status: DRAFT
authority: NONE
...
**Добавляет advanced gates для high-risk, regulated, destructive или multi-party workflows после human architecture and policy decisions.**

Основной наблюдаемый результат функции: **`Advanced Governance Gate Result`**.

`IDEA-058` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-058` family flow теряет конкретный механизм «Advanced Governance Gates», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Advanced Governance Gate Result` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: ADVANCED_GOVERNANCE_GATE_RESULT
```
...
1. Пользователь или upstream stage передаёт exact input для **Advanced Governance Gates**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-058`.
4. Создаётся **`Advanced Governance Gate Result`** с Evidence и limitations.
...
**Primary output:** `Advanced Governance Gate Result`.
...
- название **Advanced Governance Gates** и exact subject;
...
- `FR-058-01` — Функция должна выполнять только следующую ответственность: Добавляет advanced gates для high-risk, regulated, destructive или multi-party workflows после human architecture and policy decisions.
- `FR-058-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-058-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-058-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-058-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-058-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-058`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-058-01` — Пользователь понимает назначение Advanced Governance Gates без чтения implementation internals.
- `AC-058-02` — Функция создаёт ровно `Advanced Governance Gate Result` либо честный terminal report.
- `AC-058-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-058-04` — Unknowns, exclusions и blockers видимы.
- `AC-058-05` — Human authority и downstream permissions не расширены.
- `AC-058-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Advanced Governance Gate Result` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_058`
