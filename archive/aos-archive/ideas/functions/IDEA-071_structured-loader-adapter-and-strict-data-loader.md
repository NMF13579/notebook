---
document_id: AOS-ATOMIC-FUNCTION-IDEA-071
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-071
function_title: "Structured Loader Adapter and Strict Data Loader"
status: DRAFT
authority: NONE
...
**Безопасно загружает structured data с strict schema, duplicate-key detection, explicit coercion policy и preservation/rejection unknown fields.**

Основной наблюдаемый результат функции: **`Strict Loader Result`**.

`IDEA-071` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-071` family flow теряет конкретный механизм «Structured Loader Adapter and Strict Data Loader», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Strict Loader Result` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: STRICT_LOADER_RESULT
```
...
1. Пользователь или upstream stage передаёт exact input для **Structured Loader Adapter and Strict Data Loader**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-071`.
4. Создаётся **`Strict Loader Result`** с Evidence и limitations.
...
**Primary output:** `Strict Loader Result`.
...
- название **Structured Loader Adapter and Strict Data Loader** и exact subject;
...
- `FR-071-01` — Функция должна выполнять только следующую ответственность: Безопасно загружает structured data с strict schema, duplicate-key detection, explicit coercion policy и preservation/rejection unknown fields.
- `FR-071-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-071-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-071-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-071-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-071-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-071`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-071-01` — Пользователь понимает назначение Structured Loader Adapter and Strict Data Loader без чтения implementation internals.
- `AC-071-02` — Функция создаёт ровно `Strict Loader Result` либо честный terminal report.
- `AC-071-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-071-04` — Unknowns, exclusions и blockers видимы.
- `AC-071-05` — Human authority и downstream permissions не расширены.
- `AC-071-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Strict Loader Result` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_071`
