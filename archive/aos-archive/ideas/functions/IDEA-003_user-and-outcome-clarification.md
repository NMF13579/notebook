---
document_id: AOS-ATOMIC-FUNCTION-IDEA-003
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-003
function_title: "User and Outcome Clarification"
status: DRAFT
authority: NONE
...
**Уточняет реальных пользователей, их job-to-be-done и наблюдаемый desired outcome, отделяя результат от предложенного пользователем способа реализации.**

Основной наблюдаемый результат функции: **`User and Outcome Clarification`**.

`IDEA-003` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-003` family flow теряет конкретный механизм «User and Outcome Clarification», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `User and Outcome Clarification` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: USER_AND_OUTCOME_CLARIFICATION
```
...
1. Пользователь или upstream stage передаёт exact input для **User and Outcome Clarification**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-003`.
4. Создаётся **`User and Outcome Clarification`** с Evidence и limitations.
...
**Primary output:** `User and Outcome Clarification`.
...
- название **User and Outcome Clarification** и exact subject;
...
- `FR-003-01` — Функция должна выполнять только следующую ответственность: Уточняет реальных пользователей, их job-to-be-done и наблюдаемый desired outcome, отделяя результат от предложенного пользователем способа реализации.
- `FR-003-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-003-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-003-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-003-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-003-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-003`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-003-01` — Пользователь понимает назначение User and Outcome Clarification без чтения implementation internals.
- `AC-003-02` — Функция создаёт ровно `User and Outcome Clarification` либо честный terminal report.
- `AC-003-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-003-04` — Unknowns, exclusions и blockers видимы.
- `AC-003-05` — Human authority и downstream permissions не расширены.
- `AC-003-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `User and Outcome Clarification` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_003`
