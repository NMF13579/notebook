---
document_id: AOS-ATOMIC-FUNCTION-IDEA-065
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-065
function_title: "Extension / Plugin Model"
status: DRAFT
authority: NONE
...
primary_feature_family: FTR-026
related_feature_families: [FTR-026]
...
**Определяет contract для optional extensions/plugins: capabilities, permissions, lifecycle, compatibility, failure isolation и discoverability.**

Основной наблюдаемый результат функции: **`Extension / Plugin Contract`**.

`IDEA-065` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-065` family flow теряет конкретный механизм «Extension / Plugin Model», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Extension / Plugin Contract` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: EXTENSION_PLUGIN_CONTRACT
```
...
1. Пользователь или upstream stage передаёт exact input для **Extension / Plugin Model**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-065`.
4. Создаётся **`Extension / Plugin Contract`** с Evidence и limitations.
...
**Primary output:** `Extension / Plugin Contract`.
...
- название **Extension / Plugin Model** и exact subject;
...
- `FR-065-01` — Функция должна выполнять только следующую ответственность: Определяет contract для optional extensions/plugins: capabilities, permissions, lifecycle, compatibility, failure isolation и discoverability.
- `FR-065-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-065-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-065-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-065-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-065-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
- [`FTR-026` — Extension/Plugin/Capability Module Model](../features/FTR-026_extension-plugin-capability-module-model.md)

Primary family layer: `Architecture extension`. Proposed disposition: `DEFERRED`.
...
Каждый event содержит `function_id=IDEA-065`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-065-01` — Пользователь понимает назначение Extension / Plugin Model без чтения implementation internals.
- `AC-065-02` — Функция создаёт ровно `Extension / Plugin Contract` либо честный terminal report.
- `AC-065-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-065-04` — Unknowns, exclusions и blockers видимы.
- `AC-065-05` — Human authority и downstream permissions не расширены.
- `AC-065-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Extension / Plugin Contract` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_065`
