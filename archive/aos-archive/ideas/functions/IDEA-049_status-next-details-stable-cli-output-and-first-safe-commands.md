---
document_id: AOS-ATOMIC-FUNCTION-IDEA-049
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-049
function_title: "Status / Next / Details, Stable CLI Output and First Safe Commands"
status: DRAFT
authority: NONE
...
**Стандартизирует представление Status / Next / Details, CLI output и first safe commands, чтобы пользователь всегда понимал текущее состояние и один следующий шаг.**

Основной наблюдаемый результат функции: **`Status / Next / Details View`**.

`IDEA-049` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-049` family flow теряет конкретный механизм «Status / Next / Details, Stable CLI Output and First Safe Commands», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Status / Next / Details View` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: STATUS_NEXT_DETAILS_VIEW
```
...
1. Пользователь или upstream stage передаёт exact input для **Status / Next / Details, Stable CLI Output and First Safe Commands**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-049`.
4. Создаётся **`Status / Next / Details View`** с Evidence и limitations.
...
**Primary output:** `Status / Next / Details View`.
...
- название **Status / Next / Details, Stable CLI Output and First Safe Commands** и exact subject;
...
- `FR-049-01` — Функция должна выполнять только следующую ответственность: Стандартизирует представление Status / Next / Details, CLI output и first safe commands, чтобы пользователь всегда понимал текущее состояние и один следующий шаг.
- `FR-049-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-049-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-049-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-049-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-049-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-049`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-049-01` — Пользователь понимает назначение Status / Next / Details, Stable CLI Output and First Safe Commands без чтения implementation internals.
- `AC-049-02` — Функция создаёт ровно `Status / Next / Details View` либо честный terminal report.
- `AC-049-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-049-04` — Unknowns, exclusions и blockers видимы.
- `AC-049-05` — Human authority и downstream permissions не расширены.
- `AC-049-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Status / Next / Details View` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_049`
