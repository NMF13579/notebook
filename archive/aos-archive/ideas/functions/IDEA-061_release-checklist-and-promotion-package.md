---
document_id: AOS-ATOMIC-FUNCTION-IDEA-061
document_type: ATOMIC_FUNCTION_DESCRIPTION
revision: R2
function_id: IDEA-061
function_title: "Release Checklist and Promotion Package"
status: DRAFT
authority: NONE
...
**Собирает release checklist и promotion package для exact candidate, environment и target, отделяя technical readiness от human release decision.**

Основной наблюдаемый результат функции: **`Release Promotion Package`**.

`IDEA-061` — Исторический/reconstructed micro-feature ID, сохранённый из source crosswalk.
...
**Atomic focus:** без `IDEA-061` family flow теряет конкретный механизм «Release Checklist and Promotion Package», из-за чего ответственность либо выполняется вручную, либо расплывается между несколькими modules и становится непроверяемой.
...
- Создать или обновить только `Release Promotion Package` и прямо связанные derived views.
...
permissions: []
material_unknowns: []
requested_output: RELEASE_PROMOTION_PACKAGE
```
...
1. Пользователь или upstream stage передаёт exact input для **Release Checklist and Promotion Package**.
2. Функция связывает input с `subject_id`, проверяет authority/permissions и перечисляет unknowns.
3. Выполняется только bounded responsibility `IDEA-061`.
4. Создаётся **`Release Promotion Package`** с Evidence и limitations.
...
**Primary output:** `Release Promotion Package`.
...
- название **Release Checklist and Promotion Package** и exact subject;
...
- `FR-061-01` — Функция должна выполнять только следующую ответственность: Собирает release checklist и promotion package для exact candidate, environment и target, отделяя technical readiness от human release decision.
- `FR-061-02` — Exact subject, input revision, provenance и temporal scope должны сохраняться в output.
- `FR-061-03` — Missing material data должны иметь status `UNKNOWN`, `BLOCKED` или `HUMAN_REVIEW_REQUIRED`, а не заполняться догадкой.
- `FR-061-04` — Technical result не должен содержать или имитировать human acceptance/authorization.
- `FR-061-05` — На failure, denied action или unresolved conflict функция выпускает report и останавливается; automatic retry запрещён.
- `FR-061-06` — Derived UI/index/report должен ссылаться на owner facts и быть rebuildable.
...
Каждый event содержит `function_id=IDEA-061`, `subject_id`, actor/tool, timestamp, status и correlation ID.

## 19. Acceptance criteria — `DESIGNED`, tests `NOT_RUN`

- `AC-061-01` — Пользователь понимает назначение Release Checklist and Promotion Package без чтения implementation internals.
- `AC-061-02` — Функция создаёт ровно `Release Promotion Package` либо честный terminal report.
- `AC-061-03` — Result связан с exact subject, source revisions и Evidence.
- `AC-061-04` — Unknowns, exclusions и blockers видимы.
- `AC-061-05` — Human authority и downstream permissions не расширены.
- `AC-061-06` — Failure behavior соответствует report + stop.
...
- При валидных входах создаётся `Release Promotion Package` для exact subject.
...
`next_required_action: HUMAN_REVIEW_OF_IDEA_061`
