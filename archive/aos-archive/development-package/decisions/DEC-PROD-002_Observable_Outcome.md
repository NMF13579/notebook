...
exact_subject: Observable outcome of the initial AOS Core v1 vertical slice
created: '2026-07-30'
gate_id: HUMAN_PRODUCT_GATE_G1
human_disposition: ACCEPT
human_decision_record:
  schema_version: aos.decision/v1
  decision_id: DEC-PROD-002
...
limitations:
  - The outcome is a reviewed DRAFT, not implementation, runtime proof, or human acceptance of the product.
  - Chat confirmation has no independent cryptographic signature.
implementation_authorization: NONE
git_authorization: NONE
---

# DEC-PROD-002 — Observable Outcome

## Decision

```yaml
observable_outcome:
  A reviewed DRAFT Product Spec and feature-specific Feature Passport
  with visible assumptions, unknowns, scope, non-goals, and one explicit
  human decision request.
human_disposition: ACCEPT
```

The result must be understandable without chat history and must stop before architecture, Task compilation, execution, or Git delivery.

## Non-claims

```text
Reviewed DRAFT ≠ accepted product contract
Reviewed DRAFT ≠ implementation readiness
Reviewed DRAFT ≠ runtime validation
Reviewed DRAFT ≠ Git authority
```
...
stale_when:
  - observable outcome changes
  - candidate binding changes without an explicit rebind
historical_next_action_at_decision_time: USE_AS_STAGE_B_WORKFLOW_TERMINAL_OUTCOME
