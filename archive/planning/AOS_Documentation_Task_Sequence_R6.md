...
correction_basis:
  prompt_revision: R2
  prompt_audit_sha256: 4d612aa28cf4b7573e19bffbe04e1c273fceccff17c4b83d6c7e4997eb31443b
---

# AOS — Последовательность создания документации по задачам R6
...
| Finding `R3 Audit` | Исправление, сохранённое в `R6` |
|---|---|
...
| Documentation sequence and dependency order | `R6` only after human acceptance |
...
Это отдельный read-only run после каждого terminal documentation Stage Report.
...
automatic_next_task_start: FORBIDDEN
...
Persistence не входит в `VALIDATE` и допускается только как отдельный, явно разрешённый `EXECUTE` run.
...
`RECORD_VERIFICATION_RESULT` не выполняется этим roadmap-placement run и не получает authority из verification result.
...
### 7.1. Canonical post-stop documentation validation
...
→ STOP
→ verification state becomes VERIFICATION_PENDING
→ separate new invocation:
    stage = VALIDATE
    validation_profile = POST_STOP_DOCUMENTATION
→ non-durable Verification Report returned in terminal response
→ STOP
→ separate persistence/correction/human/next-task route
```

`POST_STOP_DOCUMENTATION` — validation profile, а не canonical stage. Новый stage `POST_STOP_VERIFY` не вводится. Проверка обязательна после каждого terminal documentation Stage Report: successful output, partial output, `FAIL`, `BLOCKED`, material `UNKNOWN` и no output artifact.
...
Отдельный `VALIDATE` run:

- read-only и zero-write;
- не изменяет subject и repository files;
- не исправляет findings;
- не запускает следующую task;
- не создаёт human decision;
- не создаёт implementation или Git authorization;
- возвращает один non-durable terminal Verification Report;
- останавливается.

Verification `PASS` означает только, что declared required checks прошли для exact subject, profile и dependency identity. Human gates и external gates остаются отдельными. Persistence результата не является частью `VALIDATE`. Verification Report не запускает рекурсивную post-stop validation: его schema проверяется внутри того же `VALIDATE` stage.

### 7.2. Persistent verification lifecycle
...
Planned single durable owner after roadmap activation:
...
```yaml
current_task_id:
task_report_identity:
output_subject:
  path:
  sha256:
  presence: PRESENT | PARTIAL | ABSENT
verification_subject_kind:
verification_profile:
  path_or_inline_id:
  sha256:
verification_status:
verification_report_identity:
verification_report_persistence: NOT_RUN | RECORDED
invalidated_by: []
next_transition:
```
...
- documentation task Stage Report proposes `verification_status: VERIFICATION_PENDING`;
- read-only `VALIDATE` cannot update durable state;
- durable transition recording requires separately authorized `RECORD_VERIFICATION_RESULT`;
- until a validation result is recorded or explicitly accepted as current by the coordinator, the next documentation task is forbidden;
- current-state writes must be atomic and owned by one artifact;
- this contract does not authorize creating or modifying `planning/CURRENT.md`.

### 7.3. Bootstrap independence

Inline bootstrap rules apply only to outputs of `SEQ-DOC-001` and `VERIFY-DOC-001`:

```text
R6 inline bootstrap rules
→ separate VALIDATE of exact subject
→ author agent and verifier agent identities recorded
→ independent verifier required for VERIFY-DOC-001
→ human review/decision on exact validation contract
...
DRAFT
→ HUMAN_REVIEW_REQUIRED
→ HUMAN_ACCEPTED | NEEDS_CHANGES | REJECTED | DEFERRED
→ SUPERSEDED
```

Human-decision tokens remain on the separate human-decision axis. Profile lifecycle is not implementation authorization.
...
Risk-scaled independence:

- `L0` may use deterministic author self-check;
- `L1` requires a separate semantic `VALIDATE` run;
- `L2` requires independent-agent cold-start review;
- validation profiles, Product/Feature Contracts, task contracts, runner contracts and implementation handoff claims require `L2`.
...
post_stop_validation:
  ...
  next_task_start_before_validation_close: FORBIDDEN
...
### Technical-result aggregation

Full canonical vocabulary is used and aggregation is fail-closed:

```text
if contract itself is violated
→ CONTRACT_VIOLATION

else if any required check FAIL
→ FAIL

else if any required check BLOCKED
→ BLOCKED

else if any required check UNKNOWN
→ UNKNOWN

else if any required check NOT_RUN
→ NOT_RUN

else if all required checks PASS and a human gate is required
→ HUMAN_REVIEW_REQUIRED
...
Optional `NOT_RUN` remains visible but does not block aggregate `PASS` only when the check is explicitly optional. No technical result creates human acceptance or authorization.
...
- `NEEDS_CHANGES` and `BLOCKED` are not readiness states;
- `READY_FOR_NEXT_DOCUMENTATION_TASK` cannot cross a human gate;
- `READY_FOR_PORTABLE_TASK_DERIVATION` has no target-binding effect;
- `READY_FOR_TARGET_BOUND_TASK_BRIEF` requires current exact Target Repository Binding;
- `READY_FOR_IMPLEMENTATION_HUMAN_DECISION` is not Execution Authorization;
- `EXTERNAL_EVIDENCE_REQUIRED` identifies only the exact `X*` gate;
- required `NOT_RUN` prevents readiness dependent on that check;
- unqualified `READY`, `DONE`, `COMPLETE` and `IMPLEMENTATION_READY` are forbidden authoritative states.

### Bounded correction after validation

`VALIDATE` only detects, classifies and routes findings. Correction is a separate canonical `EXECUTE` run:

```yaml
task_class: BOUNDED_DOCUMENTATION_CORRECTION
stage: EXECUTE
single_run: true
subject_path: exact
subject_sha256: exact
finding_ids: []
stop_after_report: true
```

No new per-finding human prompt is needed only when the original documentation-task contract explicitly preauthorizes this exact policy:

```yaml
correction_policy:
  preauthorized: true
  authorization_scope: THIS_TASK_REVISION_AND_EXACT_SUBJECT_ONLY
  allowed_finding_classes:
    - STRUCTURAL_FORMAT
    - YAML_SYNTAX_NO_VALUE_CHANGE
    - MARKDOWN_FENCE_BALANCE
    - UNAMBIGUOUS_RELATIVE_PATH
    - STALE_EXACT_DIGEST_OR_IDENTIFIER
    - DETERMINISTIC_TRACEABILITY_REFERENCE
  exact_subject_path:
  allowed_paths:
    - exact subject path
  max_attempts: 1
...
Allowed correction must be mechanically unique and derived from exact accepted or directly observed inputs. New human authorization is always required when a finding affects product or architecture meaning; owner/authority; feature disposition; first-slice selection; scope/non-goals/dependencies; acceptance or negative-scenario semantics; safety/recovery/permissions; a choice among valid alternatives; target repository facts; implementation; or Git actions.

```text
VALIDATE finding
→ STOP
→ correction authorization check
→ separate EXECUTE correction run
→ Stage Report
→ STOP
→ separate VALIDATE run
```

After one failed correction attempt:

```text
automatic_or_preauthorized_correction: EXHAUSTED
→ human correction authorization required
→ STOP
```

No self-heal loop is permitted.

### Verification cache identity and invalidation

```yaml
verification_identity_inputs:
  - task_id
  - task_contract_revision_or_sha256
  - stage_report_sha256
  - subject_sha256_or_absent_marker
  - validation_profile_sha256
  - authoritative_dependency_set_sha256
  - verifier_identity_and_version
```

Suggested durable report identity, only if separately authorized:

```text
planning/reports/verification/
  <task_id>/
  <subject_or_report_sha256>/
  <profile_sha256>/
  <dependency_set_sha256>.md
```

Cached verification may be reused only when every identity input matches. It is invalidated by subject-byte change, Stage Report change, task-contract change, profile change, authoritative-owner/dependency change, automated verifier implementation/version change, a new relevant human decision, or stale target binding.

### Artifact-type matrix and cold-start test

Risk depth:

```text
L0 — deterministic identity/structure
L1 — semantic completeness and authority
L2 — independent cold-start agent usability
```

| Artifact type | Minimum depth |
|---|---|
| Navigation/owner map | L1 |
| Status/profile/index artifact | L1 |
| Product/Feature Contract | L2 |
| Portable Task Candidate contract | L2 |
| Target-Bound Task Brief | L2 |
| Runner/coordinator contract | L2 |
| Validation profile | L2 |
| Implementation handoff/readiness claim | L2 |
| Progress/checklist/report schema | L1 |

L2 cold-start test:

```text
independent agent receives only declared authoritative inputs
→ identifies purpose and bounded scope
→ identifies missing human decisions
→ reconstructs one next bounded task or declares blocker
→ derives acceptance and negative tests where applicable
→ does not use chat history
→ does not infer target facts
```

Cold-start result is validation Evidence only, not implementation authorization.
...
| `VERIFY-DOC-001` | `planning/verification/POST_STOP_DOCUMENTATION_VALIDATION_CONTRACT_R1.md` | bootstrap `VALIDATE` of `SEQ-DOC-001` |
| `SEQ-DOC-002` | `AOS_Documentation_Task_Manifest_R1.md` | `SEQ-DOC-001`, human-accepted `VERIFY-DOC-001` |
| `SEQ-DOC-003` | `AOS_Gate_Status_Usage_Profile_R1.md` | `docs/00_Core.md`, `SEQ-DOC-001`, human-accepted `VERIFY-DOC-001` |
| `SEQ-DOC-004` | `AOS_Feature_Coverage_Ledger_R1.md` | `docs/06_Features.md`, human-accepted `VERIFY-DOC-001` |
| `TASK-DOC-001` | `AOS_Portable_Task_Candidate_Contract_R1.md` | `docs/02_Architecture.md`, `docs/03_Development.md`, human-accepted `VERIFY-DOC-001` |
| `TASK-DOC-002` | `AOS_Target_Binding_And_Task_Conversion_Protocol_R1.md` | `TASK-DOC-001`, human-accepted `VERIFY-DOC-001` |
| `SEQ-DOC-005` | `AOS_Documentation_Progress_Checklist_R2.md` | `SEQ-DOC-002…004`, human-accepted `VERIFY-DOC-001` |
...
review_criteria:
  ...
  - R6 is used only for sequence ownership after human acceptance
...
post_stop_validation:
  required: true
  canonical_stage: VALIDATE
  validation_profile: BOOTSTRAP_INLINE_R6_DOCUMENTATION_VALIDATION
  subject_selection:
    - OUTPUT_ARTIFACT_AND_STAGE_REPORT
    - PARTIAL_ARTIFACT_AND_STAGE_REPORT
    - STAGE_REPORT_ONLY
...
correction_policy:
  preauthorized: false
  exact_subject_path: planning/AOS_Authoritative_Owner_Map_R1.md
  allowed_finding_classes: []
  max_attempts: 0
  scope_expansion: FORBIDDEN
  semantic_change: FORBIDDEN
...
implementation_authorization: NONE
git_authorization: NONE
next_required_action: INDEPENDENT_VALIDATE_EXACT_SEQ_DOC_001_OUTPUT
stop: true
```
...
```yaml
task_id: VERIFY-DOC-001
work_item_type: DOCUMENTATION_TASK
title: Create Post-Stop Documentation Validation Contract
status: DRAFT

purpose: >
  Определить canonical VALIDATE profile, Verification Report schema, readiness
  matrix, lifecycle state, persistence boundary and bounded correction policy
...
required_sections:
  - purpose_and_scope
  - canonical_stage_and_profile
  - trigger_and_subject_selection
  - deterministic_checks
  - semantic_checks
  - authority_and_safety_checks
  - risk_scaled_validation_depth
  - artifact_type_matrix
  - technical_result_aggregation
  - readiness_axis
  - verification_report_schema
  - lifecycle_state_machine
  - non_durable_report_boundary
  - durable_persistence_protocol
  - bounded_correction_policy
  - cache_identity_and_invalidation
  - cold_start_agent_usability_test
  - transition_rules
  - failure_and_blocker_routes

review_criteria:
  - canonical stage remains VALIDATE
  - validator is read-only and zero-write
  - all terminal Stage Reports are covered
  - Stage Report-only validation is supported
  - technical result, human decision and readiness are independent
  - required NOT_RUN prevents dependent readiness
  - verifier cannot fix subject
  - correction requires a separate EXECUTE run
  - only preauthorized mechanical defects may be corrected without a new prompt
  - correction attempts are bounded
  - persistent state has one owner
  - normal use requires independent validation and human activation of profile
  - cold-start agent-usability test is defined
  - report identity includes profile and dependency identities
  - each run ends with report and stop

negative_scenarios:
  - new noncanonical stage introduced
  - validator writes repository
  - validator fixes subject
  - blocked task without output skips validation
  - PASS used as human acceptance
  - readiness used as authorization
  - auto-correction changes product or architecture meaning
  - infinite correction loop
  - stale validation result reused
  - profile validates and activates itself
  - next task starts before validation route closes
...
correction_policy:
  preauthorized: false
  exact_subject_path: planning/verification/POST_STOP_DOCUMENTATION_VALIDATION_CONTRACT_R1.md
  allowed_finding_classes: []
  max_attempts: 0
  scope_expansion: FORBIDDEN
  semantic_change: FORBIDDEN
...
implementation_authorization: NONE
git_authorization: NONE
next_required_action: INDEPENDENT_VALIDATE_EXACT_VERIFY_DOC_001_OUTPUT
stop: true
```
...
```text
SEQ-DOC-001
→ bootstrap VALIDATE of SEQ-DOC-001 output
→ VERIFY-DOC-001
→ independent VALIDATE of exact verification contract
→ human decision on exact verification contract
→ remaining W0 documentation tasks
```

Normal `POST_STOP_DOCUMENTATION` profile use requires exact human acceptance of the independently validated contract. No active verification profile is created by R6.
...
- owner map reviewed;
- bootstrap validation of the exact owner map completed;
- exact verification contract independently validated;
- exact verification contract human decision is `ACCEPT`;
- manifest acyclic;
...
## 23.1. Conditional validation automation documentation

Manual validation is required first; dispatcher implementation is not claimed.
...
title: Create Validation Dispatch and Persistence Automation Contract

activation_requirements:
  - accepted POST_STOP_DOCUMENTATION validation contract
  - repeated successful manual validation cycles
  - known failure modes
  - bounded correction evidence
  - fallback and removal path
  - separate human automation-admission decision

required_contract_fields:
  - trigger_event
  - trigger_owner
  - durable_pending_state
  - dedupe_key
  - lock_scope
  - max_attempts
  - retry_policy
  - no_retry_on_material_failure
  - persistence_authorization
  - correction_authorization_check
  - fallback
  - removal
  - observability
```

This conditional task does not block `W0–W6`, does not authorize implementation, and does not authorize automatic dispatch or persistence.
...
После human acceptance `R6` необходимо создать:
...
- принимать `R6` без exact human decision;
...
## 29A. R6 structured semantic-diff boundary

```yaml
source_revision: R5
target_revision: R6
allowed_change_classes:
  - frontmatter revision/correction fields
  - post-stop VALIDATE profile model
  - verification work-item definition
  - verification lifecycle state model
  - W0 verification contract task
  - optional later automation-admission task
  - generic documentation-task validation fields
  - verification report schema
  - technical-result aggregation rules
  - readiness axis and reasons
  - bounded correction policy
  - report-persistence separation
  - cache identity and invalidation rules
  - risk-scaled validation depth
  - cold-start agent-usability test
  - minimal dependency/transition references required by these additions
preserved_without_semantic_change:
  - exact authoritative owner routes
  - historical roadmap boundary
  - conditional activation state
  - bounded SEQ-DOC-001 contract except validation-field integration
  - W0_W8_X1_X4 ordering
  - feature coverage and dispositions
  - first-slice sequence
  - canonical stage vocabulary
  - one-run-one-stage invariant
  - external-gate boundaries
  - extension placement
  - implementation and Git authority
unexpected_changes: []
```

## 29. Correction closure matrix

| Audit finding | R6 closure |
|---|---|
...
Провести read-only audit exact `AOS_Documentation_Task_Sequence_R6.md`.
